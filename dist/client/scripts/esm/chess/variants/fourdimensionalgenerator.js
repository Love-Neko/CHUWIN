...
// Utility ---------------------------------------------------------------------------------------------------------
function set4DBoardDimensions(boards_x, boards_y, board_spacing) {
...
function setMovementType(strong_kings_and_queens, strong_pawns) {
 ...
 * @returns
 */
function gen4DPosition(boards_x, boards_y, board_spacing, input_position) {
    ...
    if (typeof input_position === 'string') {
        const input_position_long = formatconverter.ShortToLong_Format(input_position).startingPosition;
            ...
            for (let j = dim.MIN_Y; j <= dim.MAX_Y; j++) {
                ...
                if ((i % dim.BOARD_SPACING === 0 || i % dim.BOARD_SPACING === 9)
                    ...
                    if ((i < dim.MAX_X) && (i % dim.BOARD_SPACING === 0) && (j < dim.MAX_Y) && (j % dim.BOARD_SPACING === 0)) {
                        ...
                        const sub_position_long = sub_position_short ? formatconverter.ShortToLong_Format(sub_position_short).startingPosition : new Map();
 ...
 * @returns
 */
function gen4DMoveset(boards_x, boards_y, board_spacing, strong_kings_and_queens, strong_pawns) {
    set4DBoardDimensions(boards_x, boards_y, board_spacing);
        ...
        for (let baseV = 1; baseV >= -1; baseV--) {
            for (let offsetH = 1; offsetH >= -1; offsetH--) {
                for (let offsetV = 1; offsetV >= -1; offsetV--) {
                    ...
                    if (x < 0)
                        continue; // If the x coordinate is negative, skip this iteration
                    if (x === 0 && y <= 0)
                        continue; // Skip if x is 0 and y is negative
                    // Add the moves
                    // allow any queen move if STRONG_KINGS_AND_QUEENS, else group her with bishops and rooks
                    if (mov.STRONG_KINGS_AND_QUEENS)
                        movesets[r.QUEEN].sliding[coordutil.getKeyFromCoords([x, y])] = [-Infinity, Infinity];
 ...
 * @returns
 */
function getPawnVicinity(board_spacing, strong_pawns) {
        ...
        for (let baseV = 1; baseV >= -1; baseV--) {
            for (let offsetH = 1; offsetH >= -1; offsetH--) {
                for (let offsetV = 1; offsetV >= -1; offsetV--) {
                    ...
                    if (baseH * baseH + offsetH * offsetH === 2)
                        continue;
                    // do not allow two moves sideways
                    if (baseV * baseV + offsetV * offsetV === 2)
                        continue;
                    // disallow strong captures if pawns are weak
                    if (!strong_pawns && (Math.abs(baseH) !== Math.abs(baseV) || Math.abs(offsetH) !== Math.abs(offsetV)))
                        continue;
 ...
 * @returns
 */
function getKnightVicinity(board_spacing) {
    const individualMoves = [];
 ...
 * @returns
 */
function getKingVicinity(board_spacing, strong_kings_and_queens) {
    const individualMoves = [];
    for (let baseH = 1; baseH >= -1; baseH--) {
        for (let baseV = 1; baseV >= -1; baseV--) {
            for (let offsetH = 1; offsetH >= -1; offsetH--) {
                for (let offsetV = 1; offsetV >= -1; offsetV--) {
                    ...
                    if (!strong_kings_and_queens && baseH * baseH + baseV * baseV + offsetH * offsetH + offsetV * offsetV > 2)
                        continue;
                    const x = board_spacing * baseH + offsetH;
                    const y = board_spacing * baseV + offsetV;
                    if (x === 0 && y === 0)
