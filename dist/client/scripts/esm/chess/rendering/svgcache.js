 ...
 */
async function getSVGElements(ids, width, height) {
 ...
 */
async function fetchMissingTypes(locations) {
    ...
    if (!processingCache[url]) {
        processingCache[url] = (async () => {
            try {
                ...
                if (!response.ok)
                    throw new Error(`HTTP error when fetching piece svgs from location "${location}"! status: ${response.status}`);
 ...
 * @param color
 */
function tintSVG(svgElement, color) {
 ...
 */
function getSVGColorPriority(color) {
    switch (color) {
 ...
 */
function getSVGIDs(types, width, height) {
    let failed = false;
    const svgs = [];
    l: for (const type of types) {
        ...
        for (const c of checks) {
            ...
            const cloned = cachedPieceSVGs[id].cloneNode(true);
    ...
    if (failed)
        throw Error("SVG theme is missing ids for pieces");
    return svgs;
}
/**
