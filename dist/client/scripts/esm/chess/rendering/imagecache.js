 ...
 */
async function initImagesForGame(gamefile) {
    if (Object.keys(cachedImages).length > 0)
        throw Error("Image cache already initialized. Call deleteImageCache() when unloading games.");
    // console.log("Initializing image cache for game...");
    // 1. Determine required piece types (excluding SVG-less ones)
    const types = gamefile.existingTypes.filter((t) => !(typeutil.getRawType(t) in typeutil.SVGLESS_TYPES));
    if (types.length === 0) {
    ...
    try {
        ...
        const newCache = {}; // 'pawn-white' => HTMLImageElement
        const normalizationPromises = [];
        for (const img of initialImages) {
            // Ensure the image has an ID which corresponds to the piece type
            if (!img.id)
                throw Error("Image is missing ID after conversion from SVG.");
...
function getPieceImage(type) {
    ...
    if (!image)
        throw new Error(`Image for piece type ${typeutil.debugType(type)} not found in cache. Was initImagesForGame() called?`);
