...
function isLoadedGameViewingWhitePerspective() {
    if (!loadedGamefile)
        throw Error("Cannot ask if loaded game is from white's perspective when there isn't a loaded game.");
 ...
 */
function loadGamefile(loadOptions) {
    if (loadedGamefile)
        throw new Error("Must unloadGame() before loading a new one.");
...
function loadLogical(loadOptions) {
...
async function loadGraphical(loadOptions) {
    ...
    if (lastmove !== undefined && !lastmove.isNull)
        movepiece.applyMove(loadedGamefile, lastmove, false); // Rewind one move
    ...
    if (lastmove !== undefined && !lastmove.isNull)
        animateLastMoveTimeoutID = setTimeout(() => {
            if (moveutil.areWeViewingLatestMove(loadedGamefile))
                return; // Already viewing the lastest move
...
function unloadGame() {
    ...
    guigameover.close(); // Nothing to report once the game it described is gone
...
function callbackCopy(event) {
    if (document.activeElement !== document.body)
        return; // Don't paste if the user is typing in an input field
...
function concludeGame() {
    if (!loadedGamefile)
        throw Error("Cannot conclude game when there isn't one loaded");
    if (loadedGamefile.gameConclusion === false)
    ...
    const victor = winconutil.getVictorAndConditionFromGameConclusion(loadedGamefile.gameConclusion).victor; // undefined if aborted
    const delayToPlayConcludeSoundSecs = 0.65;
    if (!onlinegame.areInOnlineGame()) {
        if (victor !== players.NEUTRAL)
            sound.playSound_win(delayToPlayConcludeSoundSecs);
        else
            sound.playSound_draw(delayToPlayConcludeSoundSecs);
    }
    else { // In online game
        if (victor === onlinegame.getOurColor())
            sound.playSound_win(delayToPlayConcludeSoundSecs);
        else if (victor === players.NEUTRAL || !victor)
            sound.playSound_draw(delayToPlayConcludeSoundSecs);
        else
            sound.playSound_loss(delayToPlayConcludeSoundSecs);
    }
    ...
    /**
     * The game over screen, with the rematch and main menu buttons.
     * Delayed so the final position (and the win/loss sound) land first, and guarded
     * because the user can reach the main menu inside that window.
     */
    setTimeout(() => {
        if (loadedGamefile === undefined || loadedGamefile.gameConclusion === false)
            return;
        guigameover.open(loadedGamefile.gameConclusion);
    }, delayToPlayConcludeSoundSecs * 1000 + 250);
}
