...
function genCheckHighlightModel(royalsInCheck) {
    ...
    const colorOfPerimeter = [color[0], color[1], color[2], 0]; // Same color, but zero opacity
    ...
    for (let i = 0; i < royalsInCheck.length; i++) {
        ...
        const dataCircle = bufferdata.getDataCircle_TRIANGLES(x, y, inRad, resolution, color);
        const dataRing = bufferdata.getDataRing(x, y, inRad, outRad, resolution, color, colorOfPerimeter);
