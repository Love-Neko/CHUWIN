 ...
 */
function makeMove(gamefile, moveDraft, { doGameOverChecks = true } = {}) {
     ...
     */
    if (gamefile.pieces.newlyRegenerated)
 ...
 */
function viewMove(gamefile, move, forward = true) {
 ...
 */
function viewIndex(gamefile, index) {
    movepiece.goToMove(gamefile, index, (move) => viewMove(gamefile, move, index >= gamefile.state.local.moveIndex));
    updateGui(false);
}
/**
 * Makes the game view the last move
 */
function viewFront(gamefile) {
 ...
 */
function navigateMove(gamefile, forward) {
 ...
 */
function animateMove(move, forward = true, animateMain = true) {
    ...
    for (const change of move.changes) {
        if (!(change.action in funcs))
            continue; // There is no animation change function for this type of Change
...
function updateGui(showMoveCounter) {
    if (showMoveCounter)
        ...
        stats.updateTextContentOfMoves(); // While we may not be OPENING the move counter, if it WAS already open we should still update the number!
