...
const timeUntilAFKSecs = 40; // 40 + 20 = 1 minute
...
const timeUntilAFKSecs_Abortable = 20; // 20 + 20 = 40 seconds
...
const timeUntilAFKSecs_Untimed = 100; // 100 + 20 = 2 minutes
...
const timerToLossFromAFK = 20000; // HAS TO MATCH SERVER-END
...
function updateAFK() {
    ...
    if (!listener_overlay.atleastOneInput() && !listener_document.atleastOneInput())
        return; // No input this frame, don't reset the timer to tell the server we are afk.
    // There has been mouse movement, restart the afk auto-resign timer.
    if (isOurAFKAutoResignTimerRunning())
        tellServerWeBackFromAFK(); // Also tell the server we are back, IF it had started an auto-resign timer!
    rescheduleAlertServerWeAFK();
}
/**
...
function rescheduleAlertServerWeAFK() {
    ...
    if (!onlinegame.isItOurTurn() || gamefileutility.isGameOver(gamefile) || onlinegame.getIsPrivate() && gamefile.untimed)
        return;
    // TEMPORARY: Timed resignable games cannot be auto-resigned from going afk (to make tournament games more fair)
    if (!gamefile.untimed && moveutil.isGameResignable(gamefile))
        return;
...
function playStaccatoNote(note, secsRemaining) {
    if (note === 'c3')
        sound.playSound_viola_c3();
    else if (note === 'c4')
        sound.playSound_violin_c4();
