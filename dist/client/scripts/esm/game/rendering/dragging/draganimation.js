 ...
 */
function pickUpPiece(piece, resetParity) {
    if (!preferences.getDragEnabled())
        return; // Dragging is disabled
    areDragging = true;
    if (resetParity)
        parity = true;
    ...
    if (animation.animations.some(a => coordutil.areCoordsEqual_noValidate(piece.coords, a.path[a.path.length - 1])))
        animation.clearAnimations(true);
}
function getRespectiveListener() {
...
function setDragLocationAndHoverSquare(worldLoc, hoverSquare) {
    worldLocation = worldLoc;
    hoveredCoords = hoverSquare;
}
/** Whether the pointer dragging the selected piece has released yet. */
function hasPointerReleased() {
    if (!areDragging)
        throw Error("Don't call hasPointerReleased() when not dragging a piece");
    const respectiveListener = getRespectiveListener();
...
function getPointerId() {
    if (!areDragging)
        throw Error("Don't call getPointerId() when not dragging a piece");
    return pointerId;
}
/**
...
function renderPiece() {
    if (!areDragging || perspective.isLookingUp() || !worldLocation)
        return;
    const outlineModel = hoveredCoords !== undefined ? genOutlineModel() : genIntersectingLines();
    outlineModel.render();
...
function genPieceModel() {
    if (perspective.isLookingUp())
        return;
    if (typeutil.SVGLESS_TYPES.some((type) => typeutil.getRawType(pieceType) === type))
        return; // No SVG/texture for this piece (void), can't render it.
    ...
    if (perspectiveEnabled)
        data.push(...bufferdata.getDataQuad_ColorTexture3D(left, bottom, right, top, z, texleft, texbottom, texright, textop, ...perspectiveConfigs.shadowColor)); // Shadow
...
function genOutlineModel() {
    ...
    if (!coordutil.areCoordsEqual(hoveredCoords, startCoords) && (pointerIsTouch || board.gtileWidth_Pixels() < minSizeToDrawOutline)) {
        ...
        if (perspective.getEnabled()) {
            ...
            boundingBox = camera.getScreenBoundingBox(false);
...
function getBoxFrameData(coords) {
    ...
    function addRectangle(x1, y1, x2, y2, x3, y3, x4, y4) {
        vertices.push(x1, y1, r, g, b, a, // Triangle 1, Vertex 1
    ...
    // Right edge
    addRectangle(outerRight, outerTop, // Outer top-right
    outerRight, outerBottom, // Outer bottom-right
    innerRight, innerBottom, // Inner bottom-right
    innerRight, innerTop // Inner top-right
    );
