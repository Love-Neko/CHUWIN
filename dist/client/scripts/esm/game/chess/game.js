...
function update() {
    ...
    if (gameloader.areWeLoadingGame())
        return; // If the game isn't totally finished loading, nothing is visible, only the loading animation.
    const gamefile = gameslot.getGamefile();
    if (!gamefile)
        return boardpos.update(); // On title screen. Updates the board's position and scale according to its velocity; // 
    ...
    idlenudge.update(); // Notices a player who has stopped moving, and nudges them
}
function render() {
    if (gameloader.areWeLoadingGame())
        return; // If the game isn't totally finished loading, nothing is visible, only the loading animation.
    ...
    const gamefile = gameslot.getGamefile();
    if (!gamefile)
        return; // No gamefile, on the selection menu. Only render the checkerboard and nothing else.
    /**
    ...
    webgl.executeWithDepthFunc_ALWAYS(() => {
        ...
        arrows.render();
        idleghost.render(); // Before the crosshair, which stays on top of everything
