 ...
 */
function calcIntersectionPointOfLines(A1, B1, C1, A2, B2, C2) {
    const determinant = A1 * B2 - A2 * B1;
    if (determinant === 0)
        return undefined; // Lines are parallel or identical
 ...
 */
function getLineGeneralFormFromCoordsAndVec(coords, vector) {
 ...
 */
function getLineGeneralFormFrom2Coords(coords1, coords2) {
 ...
 */
function areLinesInGeneralFormEqual(line1, line2) {
 ...
 */
function calculateVectorFromPoints(start, end) {
 ...
 */
function getLineCFromCoordsAndVec(coords, vector) {
...
function boxContainsSquare(box, square) {
    if (square[0] < box.left)
        return false;
    if (square[0] > box.right)
        return false;
    if (square[1] < box.bottom)
        return false;
    if (square[1] > box.top)
        return false;
    return true;
...
function expandBoxToContainSquare(box, coord) {
    if (coord[0] < box.left)
        box.left = coord[0];
    else if (coord[0] > box.right)
        box.right = coord[0];
    if (coord[1] < box.bottom)
        box.bottom = coord[1];
    else if (coord[1] > box.top)
        box.top = coord[1];
}
/**
 ...
 */
function calcCenterOfBoundingBox(box) {
    return [
 ...
 */
function distanceBetweenParallelLines(A, B, C1, C2) {
    if (A === 0 && B === 0)
 ...
 */
function findFarthestPointsALineSweepsABox(vector, boundingBox) {
    ...
    let farthestLines = [corners[0], corners[1]];
 ...
 */
function findLineBoxIntersections(coords, direction, box) {
    const intersections = [];
    // Function to check intersection with a vertical line (x = constant)
    function checkVerticalEdge(x) {
        if (direction[0] === 0)
            return null; // No intersection with vertical line if no x-direction
    ...
    function checkHorizontalEdge(y) {
        if (direction[1] === 0)
            return null; // No intersection with horizontal line if no y-direction
...
function areLinesCollinear(lines) {
    ...
    for (const line of lines) {
        ...
        if (gradient === undefined)
            ...
            continue;
        else if (!isAproxEqual(lgradient, gradient))
            return false;
    }
    return true;
}
 ...
 */
function getKeyFromVec2(vec2) {
    return `${vec2[0]},${vec2[1]}`;
}
/**
 * Returns the vector from the Vec2Key: 'dx,dy' => [dx,dy]
 */
function getVec2FromKey(vec2Key) {
    return vec2Key.split(',').map(Number);
}
/**
 ...
 */
function calculateVectorComponents(vector, length) {
...
function LCM(array) {
    ...
    if (array.length === 0)
        throw Error('Array of numbers must have atleast one number to calculate the LCM.');
    // Initialize result
    let answer = array[0];
 ...
 */
function isAproxEqual(a, b, epsilon = 0.000001) {
 ...
 */
function roundUpToNextPowerOf2(num) {
    if (num <= 1)
        return 1; // Handle edge case for numbers 0 and 1
