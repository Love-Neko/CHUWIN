...
function deepCopyObject(src) {
    ...
    if (src instanceof Map) {
        ...
        return copy; // Return the new Map with deep copied entries
    }
    // Check for Sets
 ...
 */
function addElementToOrganizedArray(sortedArray, value) {
    const { found, index } = binarySearch(sortedArray, value);
    if (found)
        throw Error(`Cannot add element to sorted array when it already contains the value! ${value}. List: ${JSON.stringify(sortedArray)}`);
 ...
 * @returns The index
 */
function findIndexOfPointInOrganizedArray(sortedArray, point) {
 ...
 */
function deleteElementFromOrganizedArray(sortedArray, value) {
    const { found, index } = binarySearch(sortedArray, value);
    if (!found)
        throw Error(`Cannot delete value "${value}" from organized array (not found). Array: ${JSON.stringify(sortedArray)}`);
...
function removeObjectFromArray(array, object) {
    ...
    if (index !== -1)
        ...
        throw Error(`Could not delete object from array, not found! Array: ${JSON.stringify(array)}. Object: ${object}`);
}
...
function getMissingStringsFromArray(array1, array2) {
    ...
    for (const item of array2) {
        if (!set1.has(item))
            missing.push(item); // If element from array2 is missing in array1, add it to the missing list
    }
 ...
 */
function estimateMemorySizeOf(obj) {
    const visited = new Set(); // Use a Set to track visited objects to handle cycles and prevent double-counting.
    // --- Helper Functions ---
    function roughSizeOfObject(value) {
        ...
        if (typeof value === 'boolean')
            ...
            bytes = 8 + Math.ceil(value.toString().length / 2); // Rough guess
...
function parseReviver(key, value) {
    if (typeof value === 'object' && value !== null) {
        if (value.$$type === 'Map')
            return new Map(value.value); // value.value should be an array of [key, value] pairs
        if (value.$$type === 'Set')
            return new Set(value.value); // value.value should be an array of elements
        if (value.$$type in FixedArrayInfo) {
            const constructor = FixedArrayInfo[value.$$type]; // Get the constructor
