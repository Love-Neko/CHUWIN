 ...
 */
import { createModel_Instanced, createModel_Instanced_GivenAttribInfo } from './buffermodel.js';
...
function regenAll(gamefile) {
    ...
    for (const type of gamefile.existingTypes) { // [43] pawn(white)
        if (typeutil.getRawType(type) === rawTypes.VOID)
            gamefile.mesh.types[type] = genVoidModel(gamefile, type); // Custom mesh generation logic for voids
        else
            gamefile.mesh.types[type] = genTypeModel(gamefile, type); // Normal generation logic for all pieces with a texture
    }
 ...
 */
function regenType(gamefile, type) {
 ...
 */
function genTypeModel(gamefile, type) {
    ...
    const instanceData64 = getInstanceDataForTypeRange(gamefile, gamefile.pieces.typeRanges.get(type));
    const image = imagecache.getPieceImage(type);
 ...
 * SLOWEST. Minimize calling.
 */
function genVoidModel(gamefile, type) {
    const vertexData = instancedshapes.getDataVoidSquare(preferences.getTintColorOfType(type));
    const instanceData64 = getInstanceDataForTypeRange(gamefile, gamefile.pieces.typeRanges.get(type));
    return {
        instanceData64,
 ...
 */
function getInstanceDataForTypeRange(gamefile, pieceList) {
    const instanceData64 = new Float64Array((pieceList.end - pieceList.start) * STRIDE_PER_PIECE); // Initialize with all 0's
    let currIndex = 0;
 ...
 */
function shiftModel(meshData, diffXOffset, diffYOffset) {
 ...
 */
function rotateAll(gamefile, newInverted) {
    ...
    for (const [stringType, meshData] of Object.entries(gamefile.mesh.types)) {
        ...
        if (typeutil.SVGLESS_TYPES.some(t => t === rawType))
            continue; // Skip voids and other non-textured pieces, currently they are symmetrical
        ...
        if (vertexData.length !== newVertexData.length)
            throw Error("New vertex data must be the same length as the existing! Cannot update buffer indices."); // Safety net
 ...
 */
function overwritebufferdata(gamefile, piece) {
...
function renderAll(gamefile) {
    ...
    if (!boardpos.areZoomedOut() && isOffsetOutOfRangeOfRegenRange(gamefile.mesh.offset))
        shiftAll(gamefile);
    // Test if the rotation has changed
    const correctInverted = perspective.getIsViewingBlackPerspective();
    if (gamefile.mesh.inverted !== correctInverted)
        rotateAll(gamefile, correctInverted);
    for (const meshData of Object.values(gamefile.mesh.types)) {
 ...
 */
function isOffsetOutOfRangeOfRegenRange(offset) {
    const boardPos = boardpos.getBoardPos();
    const xDiff = Math.abs(boardPos[0] - offset[0]);
    const yDiff = Math.abs(boardPos[1] - offset[1]);
    if (xDiff > REGEN_RANGE || yDiff > REGEN_RANGE)
        return true;
    return false;
}
// Exports --------------------------------------------------------------------------------------------
