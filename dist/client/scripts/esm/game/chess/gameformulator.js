 ...
 */
function formulateGame(compressedGame) {
    /** String array of the moves in their most compact notation (e.g. "4,7>4,8Q") */
    const moves = compressedGame.moves.map((m) => m.compact);
    ...
    // Optional properties
    if (compressedGame.moveRuleState !== undefined)
        variantOptions.moveRuleState = compressedGame.moveRuleState;
    if (compressedGame.enpassant) { // Coords: [x,y]
        ...
        const pawnExpectedSquare = [compressedGame.enpassant[0], compressedGame.enpassant[1] - yParity];
        const pieceOnExpectedSquare = compressedGame.startingPosition.get(coordutil.getKeyFromCoords(pawnExpectedSquare));
 ...
 */
function ICNToGamefile(ICN) {
    const longformat = formatconverter.ShortToLong_Format(ICN);
    const variantOptions = {
    ...
    if (longformat.enpassant)
        throw Error('Logged game ICNs should NEVER have an enpassant property on the starting position!!');
    /**
