...
function initPrograms() {
    programs.push(createColorProgram(), createColorProgram_Instanced(), createColorProgram_Instanced_Plus(), createTextureProgram(), createColoredTextureProgram(), createTextureProgram_Instanced(), createTintedInstancedTextureProgram(), createColoredTextureProgram_Instanced(), createTintedTextureProgram());
}
/**
 ...
 */
function createColorProgram_Instanced_Plus() {
    const vsSource = `#version 300 es
        in vec4 aVertexPosition;
 ...
 */
function createTextureProgram_Instanced() {
    const vsSource = `#version 300 es
 ...
 */
function createTintedInstancedTextureProgram() {
    const vsSource = `#version 300 es
 ...
 */
function createColoredTextureProgram_Instanced() {
    const vsSource = `#version 300 es
        in vec4 aVertexPosition;        // Per-vertex position (vec4 for homogeneous coordinates)
        in vec2 aTextureCoord;          // Per-vertex texture coordinates
        in vec3 aInstancePosition;      // Per-instance position offset (vec3: xyz)
...
function shaderPicker(attributes, uniforms = []) {
    ...
    if (compatibleShaders.length === 0)
        throw Error(`Cannot find a shader compatible with the requested attributes and uniforms: ${JSON.stringify(attributes)}, ${JSON.stringify(uniforms)}`);
    ...
    const leastComplexShader = compatibleShaders.reduce((leastComplex, current) => {
        ...
        if (leastComplexComplexity === currentComplexity)
            throw Error(`Shaders have the same level of complexity, can't pick which one to use! Requested attributes and uniforms: ${JSON.stringify(attributes)}, ${JSON.stringify(uniforms)}`);
