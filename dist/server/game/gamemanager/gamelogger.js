...
async function logGame(game) {
    ...
    if (move_count === 0)
        return; // Don't log games with zero moves
    ...
    const victor = winconutil.getVictorAndConditionFromGameConclusion(game.gameConclusion).victor;
...
async function enterGameInGamesTable(game, dateSqliteString) {
    ...
    if (!ICN)
        return { success: false, reason: `ICN undefined when logging game, cannot log or increment player stats! Game: ${gameutility.getSimplifiedGameString(game)}` };
    const terminationCode = winconutil.getVictorAndConditionFromGameConclusion(game.gameConclusion).condition;
    const game_rated = (game.rated ? 1 : 0);
    ...
    const game_private = (game.publicity !== 'public' ? 1 : 0);
    ...
    return { success: true, game_id: results.result.lastInsertRowid }; // The lastInsertRowid is the id of the game we just inserted
}
/** Converts a server-side {@link Game} into an ICN */
async function getICNOfGame(game, metadata) {
    ...
    let ICN;
    try {
        ICN = formatconverter.LongToShort_Format(primedGamefile, { compact_moves: 2, make_new_lines: false, specifyPosition: false });
    }
    catch (error) {
 ...
 */
async function updatePlayerGamesTable(game, game_id, victor) {
    for (const playerStr in game.players) {
        ...
        if (user_id === undefined)
            continue; // Guest players don't get an entry in the player_games table or an elo for updating
 ...
 */
async function updatePlayerStatsTable(game, game_id, victor) {
    ...
    for (const playerStr in game.players) {
        ...
        const user_id = game.players[playerStr].identifier.user_id;
        if (user_id === undefined)
            continue; // Guest players don't have a row in the player_stats table for updating
 ...
 */
function getPlayerMoveCountsInGame(game) {
    ...
    for (const playerStr in game.players) {
        const player = Number(playerStr);
        playerMoveCounts[player] = fullmoves_completed_total * game.gameRules.turnOrder.filter((p) => p === player).length;
        playerMoveCounts[player] += game.gameRules.turnOrder
            .slice(0, last_partial_move_length)
            .filter((p) => p === player).length;
    }
...
async function migrateGameLogsToDatabase() {
    ...
    rl.on('line', async (line) => {
        ...
        // line starts with metadata
            ...
            if (lastread_JSON.publicity !== 'public' && lastread_JSON.publicity !== 'private')
                throw Error(`Publicity "${lastread_JSON.publicity}" not valid!`);
            ...
            if (longformat.metadata.White !== "(Guest)")
                game.players[1].identifier = { member: longformat.metadata.White, user_id: uuid.base62ToBase10(longformat.metadata.WhiteID) };
            else
                game.players[1].identifier = { browser: 'examplebrowserid' };
            if (longformat.metadata.Black !== "(Guest)")
                game.players[2].identifier = { member: longformat.metadata.Black, user_id: uuid.base62ToBase10(longformat.metadata.BlackID) };
            else
                game.players[2].identifier = { browser: 'examplebrowserid' };
            game.gameRules = longformat.gameRules;
            if (longformat.metadata.Termination === undefined)
                throw Error(`Termination metadata is undefined!`);
                ...
                if (condition === undefined)
                    throw Error(`Termination "${longformat.metadata.Termination}" not valid!`);
 ...
 */
function VerifyRequiredMetadata(metadata) {
    ...
    if (metadata.White !== "(Guest)" && !metadata.WhiteID)
        console.log(`WhiteID is missing, but White is present! ${metadata.White}`);
    if (metadata.Black !== "(Guest)" && !metadata.BlackID)
        console.log(`BlackID is missing, but Black is present! ${metadata.Black}`);
    if (metadata.WhiteID && metadata.White === "(Guest)")
        console.log(`White is missing, but WhiteID is present! ${metadata.WhiteID}`);
    if (metadata.BlackID && metadata.Black === "(Guest)")
        console.log(`Black is missing, but BlackID is present! ${metadata.BlackID}`);
    // Make sure specifically TimeControl isn't in m+s, but s+s format
    if (metadata.TimeControl === '10+4' || metadata.TimeControl === '10+6')
        console.log(`Time control "${metadata.TimeControl}" not valid! Should be in s+s format.`);
    return true;
}
export default {
    logGame,
