...
const default_win_conditions = { [p.WHITE]: ['checkmate'], [p.BLACK]: ['checkmate'] };
 ...
 */
function getPieceAbbrevRegexSource(playerCapture, abbrevCapture) {
    // Capture group names must not contain special characters used in regex.
    const captureGroupNameRegex = /^[$_A-Za-z][$\w]*$/;
    if (playerCapture !== null && !captureGroupNameRegex.test(playerCapture))
        throw Error("Invalid playerCapture group name: " + playerCapture);
    if (abbrevCapture !== null && !captureGroupNameRegex.test(abbrevCapture))
        throw Error("Invalid abbrevCapture group name: " + abbrevCapture);
 ...
 */
const moveRegexSource = `(${getPieceAbbrevRegexSource(null, null)})?` + // Optional starting piece abbreviation "P"   DOESN'T NEED TO BE CAPTURED, this avoids a crash cause of duplicate capture group names
...
function isPromotionListDefaultPromotions(promotionList) {
    if (promotionList.length !== default_promotions.length)
        return false;
 ...
 * '3k' => [68] king(red)
 */
function getTypeFromAbbr(pieceAbbr) {
    const results = new RegExp(`^${getPieceAbbrevRegexSource('player', 'abbrev')}$`).exec(pieceAbbr);
    if (results === null)
        throw Error("Piece abbreviation is in invalid form: " + pieceAbbr);
    ...
    if (playerStr === undefined) { // No player number override is present
        ...
        if (typeStr === undefined)
            throw Error("Unknown piece abbreviation: " + pieceAbbr);
        return Number(typeStr);
    }
    else { // Player number override present
        const rawTypeStr = piece_codes_raw_inverted[abbrev.toLowerCase()];
        if (rawTypeStr === undefined)
            throw Error("Unknown raw piece abbreviation: " + pieceAbbr);
        return typeutil.buildType(Number(rawTypeStr), Number(playerStr));
    }
}
 ...
 */
function getCompactMoveFromDraft(moveDraft) {
 ...
 */
function getShortFormMoveFromMove(move, options) {
    if (options.compact && !options.spaces && !options.comments)
        console.warn("getCompactMoveFromDraft() is more efficient to get the most-compact form of a move.");
    ...
    if (options.comments && (move.comment || move.clk !== undefined)) {
        ...
        if (move.clk !== undefined)
            parts.push(getClkEmbededCommandSequence(move.clk)); // '[%clk 0:09:56.7]'
        // Append the comment, if present
        if (move.comment)
            parts.push(move.comment); // 'White captures en passant'
...
function parseCompactMove(compactMove) {
    const match = moveRegexCompact.exec(compactMove);
    if (match === null)
        throw Error("Invalid compact move: " + compactMove);
    return getParsedMoveFromNamedCapturedMoveGroups(match.groups).moveDraft;
}
/** Parses a shortform move in any dynamic format to a readable json. */
function parseMoveFromShortFormMove(shortFormMove) {
    const moveRegex = new RegExp(`^${moveRegexSource}$`);
    const match = moveRegex.exec(shortFormMove);
    if (match === null)
        throw Error("Invalid shortform move: " + shortFormMove);
    return getParsedMoveFromNamedCapturedMoveGroups(match.groups);
}
/**
 ...
 */
function getParsedMoveFromNamedCapturedMoveGroups(capturedGroups) {
    ...
    if (promotionAbbr)
        moveDraft.promotion = getTypeFromAbbr(promotionAbbr);
    const parsedMove = { moveDraft };
    if (comment)
        parsedMove.comment = comment;
    return parsedMove;
}
/**
 ...
 */
function getClkEmbededCommandSequence(timeRemainMillis) {
 ...
 */
function getShortFormMovesFromMoves(moves, options) {
 ...
 */
function getShortFormMovesFromMoves_MoveNumbers(moves, options) {
    /**
...
function parseShortFormMoves(shortformMoves) {
    ...
    while ((match = moveRegex.exec(shortformMoves)) !== null) {
        moves.push(getParsedMoveFromNamedCapturedMoveGroups(match.groups));
    }
 ...
 */
function getShortFormPosition(position, specialRights) {
    const pieces = []; // ['P1,2+','P2,2+', ...]
...
function generateSpecialRights(position, pawnDoublePush, castleWith) {
    ...
    if (pawnDoublePush === false && castleWith === undefined)
        return specialRights; // Early exit
    ...
    if (Object.keys(kingsFound).length === 0)
        return specialRights; // Nothing can castle, return now.
    outerFor: for (const coord in castleWithsFound) { // 'x,y': player
        ...
        for (const kingCoord in kingsFound) { // 'x,y': player
            ...
            if (coords[1] !== kingCoords[1])
                continue; // Not the same y level
            if (castleWithsFound[coord] !== kingsFound[kingCoord])
                continue; // Their players don't match
            const xDist = Math.abs(coords[0] - kingCoords[0]);
            if (xDist < 3)
                continue; // Not at least 3 squares away
 ...
 */
function generatePositionFromShortForm(shortposition) {
    ...
    while ((match = pieceRegex.exec(shortposition)) !== null) {
        ...
        const coordsKey = match.groups['coordsKey'];
...
export { 
// Dictionaries
player_codes, player_codes_inverted, piece_codes, piece_codes_inverted, piece_codes_raw, piece_codes_raw_inverted, metadata_key_ordering, default_promotions, default_win_conditions, excludedGameRules, };
export default {
