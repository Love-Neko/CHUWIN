...
function getOwnerFromSocket(ws) {
    const metadata = ws.metadata;
    if (metadata.memberInfo.signedIn)
        return { member: ws.metadata.memberInfo.username, user_id: ws.metadata.memberInfo.user_id };
    else
        return { browser: metadata.cookies['browser-id'] };
}
/**
 ...
 */
function getCookiesFromWebsocket(req) {
    ...
    for (const cookie of rawCookies.split(';')) {
        ...
        if (parts.length < 2)
            continue; // Skip if no value part exists
 ...
 */
function getIPFromWebsocketUpgradeRequest(req) {
 ...
 */
function getSignedInAndIdentifierOfSocket(ws) {
