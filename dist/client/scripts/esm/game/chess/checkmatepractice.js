...
function setUndoingIsLegal(value) {
 ...
 */
function startCheckmatePractice(checkmateSelectedID) {
...
function generateCheckmateStartingPosition(checkmateID) {
    ...
    const blackroyalnearer = checkmatesWithBlackRoyalNearer.includes(checkmateID);
    const startingPosition = new Map(); // the position to be generated
    ...
    for (const entry of piecelist) {
        let amount = parseInt(entry.match(/[0-9]+/)[0]); // number of pieces to be placed
        const strpiece = entry.match(/[a-zA-Z]+/)[0]; // piecetype to be placed
        const piece = icnconverter.getTypeFromAbbr(strpiece);
        ...
        while (amount !== 0) {
            if (typeutil.getColorFromType(piece) === players.WHITE) {
                ...
                const x = Math.floor(Math.random() * (blackroyalnearer ? 7 : 11)) - (blackroyalnearer ? 3 : 5);
                const y = Math.floor(Math.random() * (blackroyalnearer ? 7 : 11)) - (blackroyalnearer ? 3 : 5);
                ...
                const x = Math.floor(Math.random() * 3) + (blackroyalnearer ? 8 : 12);
                const y = Math.floor(Math.random() * (blackroyalnearer ? 17 : 35)) - (blackroyalnearer ? 9 : 17);
                const key = coordutil.getKeyFromCoords([x, y]);
...
function squareNotInSight(square, startingPosition) {
    ...
    for (const [key, value] of startingPosition) {
        ...
        if (x === sx || y === sy || Math.abs(sx - x) === Math.abs(sy - y))
            return false;
...
*/
function eraseCheckmatePracticeProgressFromLocalStorage() {
    ...
    if (!completedCheckmates)
        return; // Haven't open the checkmate practice menu yet, so it's not defined.
...
function updateCompletedCheckmates() {
    ...
    const cookieCheckmates = docutil.getCookieValue('checkmates_beaten');
 ...
 */
async function markCheckmateBeaten(checkmatePracticeID) {
    if (!completedCheckmates)
    ...
    if (token)
        config.headers['Authorization'] = `Bearer ${token}`; // If you use tokens for authentication
    try {
        const response = await fetch('/api/update-checkmatelist', config);
...
function onEngineGameConclude() {
    ...
    if (!inCheckmatePractice)
        return; // Not in checkmate practice
    const gameConclusion = gameslot.getGamefile().gameConclusion;
    if (gameConclusion === false)
        throw Error('Game conclusion is false, should not have called onEngineGameConclude()');
    // Did we win or lose?
    const victor = winconutil.getVictorAndConditionFromGameConclusion(gameConclusion).victor;
    if (victor === undefined)
        throw Error('Victor should never be undefined when concluding an engine game.');
    if (!(enginegame.getOurColor() === victor))
        return; // Lost
    // WON!!! 🎉
...
function registerHumanMove() {
    if (!inCheckmatePractice)
        return; // The engine game is not a checkmate practice game
...
function restartGame() {
    if (!inCheckmatePractice)
        return console.error("Restarting games is currently not supported for non-practice mode games");
