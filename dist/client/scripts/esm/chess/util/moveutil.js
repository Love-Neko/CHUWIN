...
function getMoveFromIndex(moves, index) {
    if (isIndexOutOfRange(moves, index))
        throw Error("Cannot get next move when index overflow");
    return moves[index];
}
/**
...
function hasPieceMoved(gamefile, coords) {
    return gamefile.moves.some((move) => !move.isNull && coordutil.areCoordsEqual(move.endCoords, coords));
}
...
function flagLastMoveAsMate(gamefile) {
    ...
    if (lastMove === undefined)
        return; // No moves, can't flag last move as mate (this can happen when pasting a game that's over)
...
function areMovesIn2DFormat(longmoves) {
    if (longmoves.length === 0)
        return false; // Empty, assume they are in the new 1D format
 ...
 */
function convertMovesTo1DFormat(moves) {
 ...
 */
function getColorThatPlayedMoveIndex(gamefile, index) {
