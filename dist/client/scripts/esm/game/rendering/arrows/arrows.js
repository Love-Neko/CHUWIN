...
const renderZoomLimitVirtualPixels = 12; // virtual pixels. Default: 20
...
function toggleArrows() {
    ...
    let nextMode = mode === 0 ? 1 : mode === 1 ? 2 : mode === 2 ? 3 : /* mode === 3 ? */ 0;
    // Calculate the cap
    const cap = gameslot.getGamefile().pieces.hippogonalsPresent ? 3 : 2;
    if (nextMode > cap)
        nextMode = 0; // Wrap back to zero
    setMode(nextMode);
}
/**
...
function update() {
    ...
    const slideArrowsDraft = generateArrowsDraft(boundingBoxInt, boundingBoxFloat);
 ...
 */
function generateArrowsDraft(boundingBoxInt, boundingBoxFloat) {
    /** The running list of arrows that should be visible */
    const slideArrowsDraft = {};
    const gamefile = gameslot.getGamefile();
    gamefile.pieces.slides.forEach((slide) => {
        ...
        for (const [lineKey, organizedLine] of organizedLinesOfDir) {
            ...
            if (C < containingPointsLineC[0] || C > containingPointsLineC[1])
            ...
            if (arrowsLine === undefined)
                continue;
            if (!slideArrowsDraft[slideKey])
                slideArrowsDraft[slideKey] = {}; // Make sure this exists first
 ...
 */
function calcArrowsLineDraft(gamefile, boundingBoxInt, boundingBoxFloat, slideDir, slideKey, organizedline) {
    const negDotProd = [];
    const posDotProd = [];
    /** The piece on the side that is closest to our screen. */
    let closestPosDotProd;
    /** The piece on the side that is closest to our screen. */
    let closestNegDotProd;
    ...
    if (intersections.length < 2)
        return; // Arrow line intersected screen box exactly on the corner!! Let's skip constructing this line. No arrow will be visible
    organizedline.forEach(idx => {
        const piece = boardutil.getPieceFromIdx(gamefile.pieces, idx);
        // Is the piece off-screen?
        if (math.boxContainsSquare(boundingBoxInt, piece.coords))
        ...
        if (thisPieceIntersections.length < 2)
            return; // RARE BUG. I think this is a failure of findLineBoxIntersections(). Just skip the piece when this happens.
        ...
        const arrowDraft = { piece, canSlideOntoScreen: false };
        // Update the piece that is closest to the screen box.
        if (positiveDotProduct) {
            if (closestPosDotProd === undefined)
                closestPosDotProd = arrowDraft;
            else if (piece.coords[axis] > closestPosDotProd.piece.coords[axis])
                closestPosDotProd = arrowDraft;
        }
        else { // negativeDotProduct
            if (closestNegDotProd === undefined)
                closestNegDotProd = arrowDraft;
            else if (piece.coords[axis] < closestNegDotProd.piece.coords[axis])
                closestNegDotProd = arrowDraft;
        }
        /**
        ...
        if (slideLegalLimit === undefined)
            return; // This piece can't slide along the direction of travel
        /**
        ...
        let farthestSlidePoint = positiveDotProduct ? [
        ...
        farthestSlidePoint = farthestSlidePoint.map(coord => isNaN(coord) ? 0 : coord);
        ...
        if (farthestSlidePointDist < firstIntersectionDist)
            return; // This piece cannot slide so far as to intersect the screen bounding box
        // This piece CAN slide far enough to enter our screen...
        arrowDraft.canSlideOntoScreen = true;
        // Add the piece to the arrow line
        if (positiveDotProduct)
            posDotProd.push(arrowDraft);
        else /* Opposite side */
            negDotProd.push(arrowDraft);
    });
    /**
     ...
     */
    if (closestPosDotProd !== undefined && !posDotProd.includes(closestPosDotProd) && typeutil.getRawType(closestPosDotProd.piece.type) !== rawTypes.VOID)
        posDotProd.push(closestPosDotProd);
    if (closestNegDotProd !== undefined && !negDotProd.includes(closestNegDotProd) && typeutil.getRawType(closestNegDotProd.piece.type) !== rawTypes.VOID)
        negDotProd.push(closestNegDotProd);
    if (posDotProd.length === 0 && negDotProd.length === 0)
        return; // If both are empty, return undefined
    // Now sort them.
    ...
    // console.log(left);
    // console.log(right);
    return { negDotProd, posDotProd, intersections: intersections };
}
/**
 ...
 */
function removeUnnecessaryArrows(slideArrowsDraft) {
    if (mode === 3)
        return; // Don't remove anything
    const slideExceptions = getSlideExceptions();
    for (const direction in slideArrowsDraft) {
        if (slideExceptions.includes(direction))
            continue; // Keep it anyway, our arrows mode is high enough
        // Remove types that can't slide onto the screen...
        const arrowsByDir = slideArrowsDraft[direction];
        for (const key in arrowsByDir) { // LineKey
            const line = arrowsByDir[key];
            removeTypesThatCantSlideOntoScreenFromLineDraft(line);
            if (line.negDotProd.length === 0 && line.posDotProd.length === 0)
                delete arrowsByDir[key];
        }
        if (jsutil.isEmpty(slideArrowsDraft[direction]))
            delete slideArrowsDraft[direction];
    }
}
/** Checks if a single animated arrow is needed, based on our current mode, and its direction. */
function isAnimatedArrowUnnecessary(gamefile, type, direction, dirKey) {
    if (mode === 3)
        return false; // Keep it, whether hippogonal orthogonal or diagonal
    if (mode === 2)
        return math.chebyshevDistance([0, 0], direction) !== 1; // Only keep orthogonals and diagonals, NO hippogonals.
    ...
    if (!thisPieceMoveset.sliding)
        return true; // This piece can't slide at all
    if (!thisPieceMoveset.sliding[dirKey])
        return true; // This piece can't slide ALONG the provided line
 ...
 */
function getSlideExceptions() {
    const gamefile = gameslot.getGamefile();
    let slideExceptions = [];
    // If we're in mode 2, retain all orthogonals and diagonals, EVEN if they can't slide in that direction.
    if (mode === 2)
        slideExceptions = gamefile.pieces.slides.filter((slideDir) => math.chebyshevDistance([0, 0], slideDir) === 1).map(math.getKeyFromVec2); // Filter out all hippogonal and greater vectors
    return slideExceptions;
}
function removeTypesThatCantSlideOntoScreenFromLineDraft(line) {
    ...
    if (line.negDotProd.length > 0) {
        const arrowDraft = line.negDotProd[line.negDotProd.length - 1];
        if (!arrowDraft.canSlideOntoScreen)
            line.negDotProd.pop();
    }
    if (line.posDotProd.length > 0) {
        const arrowDraft = line.posDotProd[line.posDotProd.length - 1];
        if (!arrowDraft.canSlideOntoScreen)
            line.posDotProd.pop();
    }
}
 ...
 */
function calculateSlideArrows_AndHovered(slideArrowsDraft) {
    ...
    for (const [key, value] of Object.entries(slideArrowsDraft)) {
        const vec2Key = key;
        const linesOfDirectionDraft = value;
        const slideDir = math.getVec2FromKey(vec2Key);
        const linesOfDirection = {};
 ...
 * @returns
 */
function processPiece(arrowDraft, vector, intersection, index, worldHalfWidth, mouseWorldLocation, testHover) {
    ...
    const worldLocation = space.convertCoordToWorldSpace_IgnoreSquareCenter(renderCoords);
 ...
 */
function teleportToPieceIfClicked(piece, vector) {
    if (listener_overlay.isMouseDown(Mouse.LEFT))
        listener_overlay.claimMouseDown(Mouse.LEFT); // Don't let the board be dragged by this mouse down
    if (!mouse.isMouseClicked(Mouse.LEFT))
        return; // Mouse did not click this frame
    ...
    const perpendicularSlideDir = [-vector[1], vector[0]]; // Rotates left 90deg
 ...
 */
function shiftArrow(type, still, start, end) {
    if (start === undefined && end === undefined)
        throw Error('Must provide one of either start or end coords of modified arrow.');
    if (still && end && !coordutil.areCoordsIntegers(end))
        throw Error('Cannot add a still-animated arrow to floating point coordinates.');
    if (!areArrowsActiveThisFrame())
        return; // Arrow indicators are off, nothing is visible.
    ...
    shifts.push({ type, still, start, end });
}
    ...
    shifts.forEach(shift => {
        if (shift.start) {
            ...
            if (originalPiece === undefined)
                throw Error('Arrow shift delete piece does not exist! start: ' + shift.start + ' Perhaps we are animating a move we are not viewing?');
        ...
        if (shift.end) {
            if (shift.still) {
                ...
                if (math.boxContainsSquare(boundingBoxInt, shift.end))
                    return; // On-screen, no arrows needed for the piece, no matter their vector
                const piece = { type: shift.type, coords: shift.end, index: -1 };
                const arrowDraft = { piece, canSlideOntoScreen: true };
                ...
                for (const lineKey of gamefile.pieces.lines.keys()) {
                    ...
                    if (isAnimatedArrowUnnecessary(gamefile, shift.type, line, lineKey))
                    ...
                    if (thisPieceIntersections.length < 2)
                        continue; // RARE BUG. I think this is a failure of findLineBoxIntersections(). Just skip the piece when this happens.
                    const positiveDotProduct = thisPieceIntersections[0].positiveDotProduct; // We know the dot product of both intersections will be identical, because the piece is off-screen.	
                    if (positiveDotProduct)
                        line = math.negateVector(line);
                    ...
                    const arrow = processPiece(arrowDraft, line, intersect, 0, worldHalfWidth, mouseWorldLocation, false);
                    const animatedArrow = {
                        ...arrow,
                        direction: line,
                    };
 ...
 */
function recalculateLinesThroughCoords(gamefile, coords) {
    ...
    for (const [slideKey, linegroup] of gamefile.pieces.lines) { // For each slide direction in the game...
        ...
        if (slideKey in slideArrows) {
            ...
            if (Object.keys(slideArrows[slideKey]).length === 0)
                delete slideArrows[slideKey];
        }
        ...
        if (organizedLine === undefined)
            continue; // No pieces on line, empty
        const arrowsLineDraft = calcArrowsLineDraft(gamefile, boundingBoxInt, boundingBoxFloat, slide, slideKey, organizedLine);
        if (arrowsLineDraft === undefined)
            continue; // Only intersects the corner of our screen, not visible.
...
function regenerateModelAndRender() {
    if (Object.keys(slideArrows).length === 0 && animatedArrows.length === 0)
    ...
    const vertexData_Pictures = bufferdata.getDataQuad_Texture(left, bottom, right, top, texleft, texbottom, texright, textop);
    const instanceData_Pictures = [];
    const vertexData_Arrows = getVertexDataOfArrow(halfWorldWidth);
    const instanceData_Arrows = [];
    // ADD THE DATA...
    for (const [key, value] of Object.entries(slideArrows)) {
        const vec2Key = key;
        const slideLinesOfDirection = value;
        const slideDir = math.getVec2FromKey(vec2Key);
     ...
     */
    const attribInfoInstancedPictures = {
     ...
     */
    const attribInfoInstancedArrows = {
 ...
 */
function concatData(instanceData_Pictures, instanceData_Arrows, arrow, vector, index) {
    /**
    ...
    if (index > 0)
        return; // We can skip, since it is an adjacent picture!
    /**
 ...
 */
function getVertexDataOfArrow(halfWorldWidth) {
