 ...
 */
function getDataVoidSquare(color: Color): number[] {
	const [r, g, b, a] = color;
	const seam: Color = [r * VOIDS.SEAM_DARKEN, g * VOIDS.SEAM_DARKEN, b * VOIDS.SEAM_DARKEN, a];

	// The instance sits at [0,0], but the square's own box is offset by the square center.
	const squareCenter: number = board.gsquareCenter();
