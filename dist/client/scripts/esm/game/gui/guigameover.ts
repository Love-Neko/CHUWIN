...
const element_gameoverUI: HTMLElement = document.getElementById('gameoverUI')!;
const element_headline: HTMLElement = document.getElementById('gameover-headline')!;
const element_detail: HTMLElement = document.getElementById('gameover-detail')!;
const element_rematch: HTMLButtonElement = document.getElementById('gameover-rematch') as HTMLButtonElement;
const element_mainmenu: HTMLButtonElement = document.getElementById('gameover-mainmenu') as HTMLButtonElement;
const element_dismiss: HTMLButtonElement = document.getElementById('gameover-dismiss') as HTMLButtonElement;
...
function getHeadline(conclusion: string): string {
	const { victor } = winconutil.getVictorAndConditionFromGameConclusion(conclusion);

	if (victor === players.NEUTRAL || victor === undefined) return tr('draw', 'Draw');
 ...
 */
function getOutcome(conclusion: string): 'win' | 'lose' | undefined {
	const { victor } = winconutil.getVictorAndConditionFromGameConclusion(conclusion);
	if (victor === players.NEUTRAL || victor === undefined) return undefined;

	const ourColor = getOurColorOrUndefined();
	if (ourColor === undefined) return undefined;
...
/** Whether the game over screen is currently showing. */
function areOpen(): boolean { return isOpen; }

/**
 * Shows the game over screen for the given conclusion.
 * Called once, from `gameslot.concludeGame()`.
 */
function open(conclusion: string | false): void {
	if (conclusion === false) return; // Nothing to report
...
function callback_rematch(): void {
	...
	gameloader.replayLastGame().then(() => {
		if (vsLLM) guiaicoach.open(); // The chat box is how you talk to your opponent
