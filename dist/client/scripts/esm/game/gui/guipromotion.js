 ...
 */
async function initUI(promotionsAllowed) {
...
function callback_promote(event) {
    const type = Number(event.currentTarget.id);
...
function update() {
    if (!selectionOpen)
        return;
    if (!listener_overlay.isMouseDown(Mouse.LEFT) && !listener_overlay.isMouseDown(Mouse.RIGHT) && !listener_overlay.isMouseDown(Mouse.MIDDLE))
        return;
