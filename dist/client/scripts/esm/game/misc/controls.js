...
function updateNavControls() {
    ...
    if (transition.areWeTeleporting())
        return; // Exit if teleporting
    // Keyboard
...
function detectPanning() {
    if (boarddrag.isBoardDragging())
        return; // Only pan if we aren't dragging the board
...
function accelPanVel(panVel, angleDegs) {
...
function deccelPanVel(panVel) {
    if (panVel[0] === 0 && panVel[1] === 0)
        return panVel; // Already stopped
    const rateToUse = perspective.getEnabled() ? panAccel3D : panAccel2D;
    const hyp = Math.hypot(...panVel);
    const newHyp = hyp - loadbalancer.getDeltaTime() * rateToUse;
    if (newHyp < 0)
        return [0, 0]; // Stop completely before we start going in the opposite direction
    const ratio = newHyp / hyp;
    const newPanVel = [panVel[0] * ratio, panVel[1] * ratio];
    return newPanVel;
}
...
function detectZooming() {
    ...
    if (scaling) {
        ...
        if (scaleVel > capToUse)
            scaleVel = capToUse;
        else if (scaleVel < -capToUse)
            scaleVel = -capToUse;
    }
    else {
...
function deccelerateScaleVel(scaleVel) {
    if (scaleVel === 0)
        return scaleVel; // Already stopped
...
function testOutGameToggles() {
    if (listener_document.isKeyDown('Backquote'))
        camera.toggleDebug();
    if (listener_document.isKeyDown('Digit4'))
        websocket.toggleDebug(); // Adds simulated websocket latency with high ping
    if (listener_document.isKeyDown('KeyM'))
        stats.toggleFPS();
}
/** Debug toggles that are only for in a game. */
function testInGameToggles(gamefile) {
    if (listener_document.isKeyDown('Escape'))
        guipause.toggle();
    if (listener_document.isKeyDown('Digit1'))
        selection.toggleEditMode(); // EDIT MODE TOGGLE
    ...
    if (listener_document.isKeyDown('Digit3'))
        animation.toggleDebug(); // Each animation slows down and renders continuous ribbon
    if (listener_document.isKeyDown('Digit5'))
        copypastegame.copyGame(true); // Copies the gamefile as a single position, without all the moves.
    if (listener_document.isKeyDown('Digit6'))
        specialrighthighlights.toggle(); // Highlights special rights and en passant
    if (listener_document.isKeyDown('Tab'))
        guipause.callback_ToggleArrows();
    ...
    if (listener_document.isKeyDown('KeyP'))
        miniimage.toggle();
