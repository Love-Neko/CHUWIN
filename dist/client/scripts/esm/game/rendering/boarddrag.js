...
function checkIfBoardGrabbed() {
    if (perspective.getEnabled() || transition.areWeTeleporting() || guipromotion.isUIOpen())
        return;
    ...
    for (const pointerId of [...listener_overlay.getPointersDown()]) {
        ...
        if (!boardIsGrabbed) { // First pointer
            ...
            if (fingerPixelDist_WhenBoardPinched === 0)
                throw Error('Finger pixel dist when pinching is 0');
            // Scale
...
function checkIfBoardDropped() {
    ...
    if (pointer2Id === undefined) { // 1 finger drag
        ...
        if (!pointer1Released && !pointer2Released)
            return; // Both fingers are still holding the board
        throwScale(now);
        if (pointer1Released && pointer2Released) { // Both fingers have been released
            ...
            if (pointer2Released) { // Only Pointer 2 released
                ...
                throw Error('Umm how did we get here?');
 ...
 */
function removeOldPositions(now) {
