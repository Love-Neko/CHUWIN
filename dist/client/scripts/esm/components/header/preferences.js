...
function loadPreferences() {
    ...
    const cookiePrefs = docutil.getCookieValue('preferences');
    if (cookiePrefs) {
...
async function sendPrefsToServer() {
    ...
    if (!changeWasMade)
        return; // Only send if preferences were changed
...
function preparePrefs() {
    ...
    Object.keys(prefsCopy).forEach(prefName => {
        if (clientSidePrefs.includes(prefName))
            delete prefsCopy[prefName];
    });
...
function setPremoveMode(premove_mode) {
    if (typeof premove_mode !== 'boolean')
...
function getLegalMoveHighlightColor({ isOpponentPiece, isPremove }) {
    const themeName = getTheme();
    if (isOpponentPiece)
        ...
        return themes.getPropertyOfTheme(themeName, 'legalMovesHighlightColor_Friendly');
}
...
function getTintColorOfType(type) {
    ...
    const baseColor = pieceThemes.getBaseColorForType(r, p);
    const themeName = getTheme();
    const themePieceColors = themes.getPropertyOfTheme(themeName, "pieceTheme");
    const tint = themePieceColors[p] ?? [1, 1, 1, 1];
