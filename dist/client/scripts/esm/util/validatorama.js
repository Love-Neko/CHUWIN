...
function renewSession() {
    ...
    if (timeSinceSessionIssued < minTimeToRenewSession)
        return; // Still a freshly issued session!
...
async function getAccessToken() {
    if (reqIsOut)
        await waitUntilInitialRequestBack();
    if (!memberInfo.signedIn)
        return;
...
async function waitUntilInitialRequestBack() {
    if (!reqIsOut)
        return; // If no request is out, resolve immediately
