...
import type { Advice, ProposedMove, Protocol, ReasoningEffort } from '../misc/aicoach.js';
import type { Player } from '../../chess/util/typeutil.js';
import type { AIOpponentEvent } from '../misc/enginegame.js';
...
const element_protocol = document.getElementById('aicoach-protocol') as HTMLSelectElement;
const element_url = document.getElementById('aicoach-url') as HTMLInputElement;
const element_key = document.getElementById('aicoach-key') as HTMLInputElement;
const element_model = document.getElementById('aicoach-model') as HTMLInputElement;
const element_moveModel = document.getElementById('aicoach-move-model') as HTMLInputElement;
const element_reasoning = document.getElementById('aicoach-reasoning') as HTMLSelectElement;
...
const element_auto = document.getElementById('aicoach-auto') as HTMLInputElement;
const element_autoCoach = document.getElementById('aicoach-autocoach') as HTMLInputElement;
...
const element_question = document.getElementById('aicoach-question') as HTMLTextAreaElement;
...
let lastAnalyzedMoveCount: number = -1;

/** Poll handle. Lets us avoid hooking into the move logic. */
let autoAnalyzeIntervalID: ReturnType<typeof setInterval> | undefined;
...
function open(): void {
	...
	if (advice !== undefined) renderAdvice(advice);
...
function initListeners(): void {
	...
	for (const type of SWALLOWED_EVENTS) element_aicoachUI.addEventListener(type, swallow);
}
...
function closeListeners(): void {
	...
	for (const type of SWALLOWED_EVENTS) element_aicoachUI.removeEventListener(type, swallow);
}
...
function loadConfigIntoForm(): void {
...
function saveFormIntoConfig(): boolean {
	const protocol = element_protocol.value === 'anthropic' ? 'anthropic' : 'openai';
	const effort = element_reasoning.value as ReasoningEffort;
	aicoach.saveConfig({
...
function callback_AutoChanged(): void {
	aicoach.saveConfig({ autoAnalyze: element_auto.checked });
}

function callback_AutoCoachChanged(): void {
	...
	if (element_autoCoach.checked) lastCritiquedMoveCount = -1;
}
...
function callback_Analyze(): void {
	void analyze(false);
}
 ...
 */
async function analyze(automatic: boolean): Promise<void> {
	const gamefile = gameslot.getGamefile();
	if (gamefile === undefined) return;
	if (aicoach.isRequestInFlight()) return;
	if (!aicoach.isConfigured()) {
		element_settings.classList.remove('hidden');
		setStatus(tr('needs_config', 'Enter your API URL, key and model, then press Save.'), true);
		return;
	...
	try {
		...
		if (!automatic) statustext.showStatus(message, true);
	} finally {
		setBusy(false);
	}
}
...
function setBusy(busy: boolean): void {
	(element_analyze as HTMLButtonElement).disabled = busy;
...
function startPollTimer(): void {
	if (autoAnalyzeIntervalID !== undefined) return;
...
function poll(): void {
	if (!isOpen) return stopPollTimer();
	updateAIMoveButton();
	if (element_auto.checked) pollForNewPosition();
	if (element_autoCoach.checked) pollForCritique();
}
 ...
 */
function updateAIMoveButton(): void {
	const gamefile = gameslot.getGamefile();
...
function pollForNewPosition(): void {
	const gamefile = gameslot.getGamefile();
	if (gamefile === undefined) return;
	if (gamefile.moves.length === lastAnalyzedMoveCount) return; // Nothing new
	if (gamefileutility.isGameOver(gamefile)) return;
	// In an online game there's nothing to advise until it's our move.
	if (onlinegame.areInOnlineGame() && !onlinegame.isItOurTurn()) return;
	// Same in an engine/AI game — don't coach the opponent's move.
	if (enginegame.areInEngineGame() && !enginegame.isItOurTurn()) return;
 ...
 */
function getOurColorOrUndefined(): Player | undefined {
	try {
		if (onlinegame.areInOnlineGame()) return onlinegame.getOurColor() as Player;
		if (enginegame.areInEngineGame()) return enginegame.getOurColor();
	} catch {
 ...
 */
function pollForCritique(): void {
	const gamefile = gameslot.getGamefile();
	if (gamefile === undefined) return;
	if (critiqueInFlight || askInFlight) return;
	if (aicoach.isRequestInFlight()) return; // An analysis is using the endpoint
	if (!aicoach.isConfigured()) return;
	...
	if (gamefileutility.isGameOver(gamefile)) {
		if (finalReviewDone) return;
		if (moveCount === 0) return; // Aborted before anyone played
	...
	if (moveCount === 0) return; // Nobody has played yet
	if (moveCount === lastCritiquedMoveCount) return; // Already commented on this position
	...
	if (ourColor === players.BLACK && moveCount < 2) return;

	// Wait for the opponent's reply before speaking.
	if (onlinegame.areInOnlineGame() && !onlinegame.isItOurTurn()) return;
	if (enginegame.areInEngineGame()) {
		if (!enginegame.isItOurTurn()) return;
		if (enginegame.isAIOpponentThinking()) return;
	}
 ...
 */
async function critique(final: boolean): Promise<void> {
 ...
 */
function onPlayerIdle(): void {
	void suggestForIdlePlayer();
}

async function suggestForIdlePlayer(): Promise<void> {
	if (!aicoach.isConfigured()) return; // Nothing to ask
	if (askInFlight || critiqueInFlight || aicoach.isRequestInFlight()) return;
	...
	const advice = aicoach.getLatestAdvice();
	if (advice?.bestMove === undefined) return;
 ...
 */
function describeMoveTiming(event: { elapsedMillis?: number, upstreamAttempts?: number }): string {
	if (event.elapsedMillis === undefined) return '';
 ...
 */
function callback_Reassess(): void {
...
function callback_QuestionKeydown(event: KeyboardEvent): void {
	if (event.key !== 'Enter' || event.shiftKey) return;
	event.preventDefault();
	void ask();
}

function callback_Send(): void {
	void ask();
}

function callback_ClearChat(): void {
	aicoach.clearChat();
	element_chat.replaceChildren();
 ...
 * still type at.
 */
async function maybeStartCoachSession(): Promise<void> {
	if (!aicoach.isConfigured()) return;
	...
	try {
		...
		if (greeting === undefined) {
			...
			if (failure !== undefined) setStatus(`${tr('persona_failed', 'Could not read the coach prompt file (tsc.md):')} ${failure}`, true);
...
async function ask(): Promise<void> {
	...
	askInFlight = true;
	(element_send as HTMLButtonElement).disabled = true;
	...
	try {
		...
		askInFlight = false;
		(element_send as HTMLButtonElement).disabled = false;
	}
}
...
function renderChat(): void {
	...
	for (const message of aicoach.getChatHistory()) {
		if (message.hidden) continue; // The session kickoff. The player never wrote it.
		if (message.role === 'user') appendBubble('you', tr('chat_you', 'You'), message.content);
		else appendBubble('ai', tr('chat_ai', 'AI'), message.content);
	}
}

/** Model output goes in via textContent. Never as HTML. */
function appendBubble(who: 'you' | 'ai', author: string, text: string): void {
...
function appendSystemBubble(text: string): void {
	const bubble = document.createElement('div');
 ...
 */
function setStatus(text: string, isError: boolean, busy: boolean = false): void {
...
/** `[4,1] → [4,3]` */
function formatMove(move: ProposedMove): string {
...
function renderAdvice(advice: Advice): void {
	element_results.replaceChildren();
	...
	if (advice.assessment !== '') element_results.append(section(tr('assessment', 'Assessment'), advice.assessment));
	if (advice.threats.length > 0) element_results.append(listSection(tr('threats', 'Threats against you'), advice.threats));
	...
	if (advice.bestMove !== undefined) {
		...
		if (advice.reasoning !== '') element_results.append(section(tr('reasoning', 'Why'), advice.reasoning));
	} else if (advice.reasoning !== '') {
		element_results.append(section(tr('reasoning', 'Why'), advice.reasoning));
	}

	if (advice.alternatives.length > 0) {
		const items = advice.alternatives.map((move) => (
			move.note === undefined ? formatMove(move) : `${formatMove(move)} — ${move.note}`
		));
		element_results.append(listSection(tr('alternatives', 'Alternatives'), items));
	}

	if (advice.rejectedMove !== undefined) {
		const warning = section(tr('illegal_title', 'Suggested move was illegal'),
	...
	if (advice.bestMove !== undefined) setStatus(`${tr('done', 'Recommended:')} ${formatMove(advice.bestMove)}`, false);
	else setStatus(tr('done_no_move', 'Analysis complete, no move recommended.'), false);
}
