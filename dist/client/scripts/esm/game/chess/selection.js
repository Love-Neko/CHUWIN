...
function toggleEditMode() {
    ...
    if (onlinegame.areInOnlineGame() && !legalInPrivate)
        return; // Don't toggle if in an online game
    if (enginegame.areInEngineGame())
        return; // Don't toggle if in an engine game
    editMode = !editMode;
 ...
 */
function updateHoverSquareLegal(gamefile) {
    if (!pieceSelected)
        return;
...
function testIfPieceSelected(gamefile) {
    // If we did not click, exit...
    const dragEnabled = preferences.getDragEnabled();
    if (dragEnabled && !mouse.isMouseDown(Mouse.LEFT) && !mouse.isMouseClicked(Mouse.LEFT))
        return; // If dragging is enabled, all we need is pointer down event.
    else if (!dragEnabled && !mouse.isMouseClicked(Mouse.LEFT))
        return; // When dragging is off, we actually need a pointer click.
    if (boardpos.boardHasMomentum())
        return; // Don't select a piece if the board is moving
    ...
    if (selectionLevel === 0)
        ...
        if (pieceSelected && hoverSquareLegal)
            return; // Return. Don't select it, NOR make the move, let testIfPieceMoved() catch that.
        // If we are viewing past moves, forward to front instead!!
        if (viewFrontIfNotViewingLatestMove(gamefile))
            return; // Forwarded to front, DON'T select the piece.
        ...
        if (listener_document.isKeyHeld('ControlLeft'))
            return; // Control key force drags the board, disallowing picking up a piece.
        // If this is the second total pointer, then skip picking it up so that board dragging can pinch the board!
        if (Object.keys(listener_overlay.getAllPointers()).length === 2)
            return;
        /** Just quickly make sure that, if we already have selected a piece,
         * AND we just clicked a piece that's legal to MOVE to,
         * that we don't select it instead! */
...
function testIfPieceDropped(gamefile) {
    if (!pieceSelected)
        return; // No piece selected, can't move nor drop anything.
    if (!draganimation.areDraggingPiece())
        return; // The selected piece is not being dragged.
    ...
    if (listener_overlay.isMouseDown(Mouse.LEFT) && listener_overlay.getPointerCount() === 2) { // Prevent accidental dragging when trying to zoom.
        ...
        if (draganimation.getDragParity())
            return unselectPiece();
        return draganimation.dropPiece();
    }
    if (!draganimation.hasPointerReleased())
        return; // The pointer has not released yet, don't drop it.
    ...
    if (dropArrowsCaptureCoords)
        return moveGamefilePiece(gamefile, dropArrowsCaptureCoords);
    ...
    if (droppedOnOwnSquare && !draganimation.getDragParity())
        unselectPiece();
    else if (hoverSquareLegal)
        moveGamefilePiece(gamefile, hoverSquare); // It was dropped on a legal square. Make the move. Making a move automatically deselects the piece and cancels the drag.
    else
        draganimation.dropPiece(); // Drop it without moving it.
}
/** If a piece is selected, and we clicked a legal square to move to, this will make the move. */
function testIfPieceMoved(gamefile) {
    if (!pieceSelected)
        return;
    if (!mouse.isMouseClicked(Mouse.LEFT))
        return; // Pointer did not click, couldn't have moved a piece.
    if (!hoverSquareLegal)
        return; // Don't move it
    else
        moveGamefilePiece(gamefile, hoverSquare);
}
/** Forwards to the front of the game if we're viewing history, and returns true if we did. */
function viewFrontIfNotViewingLatestMove(gamefile) {
 ...
 */
function canSelectPieceType(gamefile, type) {
    if (type === undefined)
        return 0; // Can't select nothing
    const [raw, player] = typeutil.splitType(type);
    if (raw === rawTypes.VOID)
        return 0; // Can't select voids
    if (editMode)
        return preferences.getDragEnabled() ? 2 : 1; // Edit mode allows any piece besides voids to be selected and dragged.
    if (player === players.NEUTRAL)
        return 0; // Can't select neutrals, period.
    if (isOpponentType(gamefile, type))
        return 1; // Can select opponent pieces, but not draggable..
    const isOurTurn = gameloader.isItOurTurn(player);
    if (!isOurTurn)
        return 1; // Can select our piece when it's not our turn, but not draggable.
    ...
    if (boarddrag.isBoardDragging() && pointerCount === 1)
        return 1;
...
function canMovePieceType(pieceType) {
    if (editMode)
        return true; // Edit mode allows pieces to be moved on any turn.
    const isOpponentPiece = isOpponentType(gameslot.getGamefile(), pieceType);
    if (isOpponentPiece)
        return false; // Don't move opponent pieces
 ...
 */
function canDropOnPieceTypeInEditMode(type) {
    if (type === undefined)
        return true; // Can drop on empty squares.
...
function isOpponentType(gamefile, type) {
 ...
 */
function selectPiece(gamefile, piece, drag) {
...
function reselectPiece() {
    ...
    if (gamefileutility.isGameOver(gamefile))
        return; // Don't reselect, game is over
...
function initSelectedPieceInfo(gamefile, piece) {
    // Initiate
    pieceSelected = piece;
 ...
 */
function moveGamefilePiece(gamefile, coords) {
...
function makePromotionMove(gamefile) {
...
function renderGhostPiece() {
    if (!pieceSelected || !hoverSquareLegal || draganimation.areDraggingPiece() || listener_overlay.isMouseTouch(Mouse.LEFT) || config.VIDEO_MODE)
        return;
    const rawType = typeutil.getRawType(pieceSelected.type);
    if (typeutil.SVGLESS_TYPES.some((type) => type === rawType))
        return; // No svg/texture for this piece (void), don't render the ghost image.
