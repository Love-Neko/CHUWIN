...
const oneWayActions = ['capture', 'delete'];
/**
 ...
 */
function queueCapture(changes, main, piece, endCoords, capturedPiece, path) {
    const change = { action: 'capture', main, piece, endCoords, capturedPiece };
    if (path !== undefined)
        change.path = path;
    changes.push(change);
    return changes;
 ...
 * @param main - Whether this change is affecting the main piece moved, not a secondary piece.
 */
function queueDeletePiece(changes, main, piece) {
 ...
 * @param endCoords
 */
function queueMovePiece(changes, main, piece, endCoords, path) {
    const change = { action: 'move', main, piece, endCoords };
    if (path !== undefined)
        change.path = path;
    changes.push(change);
    return changes;
 ...
 */
function runChanges(gamefile, changes, changeFuncs, forward = true) {
 ...
 */
function applyChanges(gamefile, changes, funcs, forward) {
    if (forward) {
        // Iterate forwards through the changes array
        for (const change of changes) {
            if (!(change.action in funcs))
                throw Error(`Missing change function for likely-invalid change action "${change.action}"!`);
 ...
 */
function addPiece(gamefile, change) {
    const pieces = gamefile.pieces;
    const typedata = pieces.typeRanges.get(change.piece.type);
    if (typedata === undefined)
        throw Error(`Type: "${typeutil.debugType(change.piece.type)}" is not expected to be in the game`);
    let idx;
    if (change.piece.index === -1) { // Does not have an index yet, assign it one from undefined list
        if (typedata.undefineds.length === 0) {
            if (organizedpieces.getTypeUndefinedsBehavior(change.piece.type, gamefile.gameRules.promotionsAllowed, gamefile.editor) === 0)
                throw Error(`Type: ${change.piece.type} is not expected to be added after initial position!`);
        ...
        if (!found)
            throw Error(`Piece ${change.piece} attemped to overwrite an occupied index`);
 ...
 */
function deletePiece(gamefile, change) {
    const pieces = gamefile.pieces;
    const typedata = pieces.typeRanges.get(change.piece.type);
    if (typedata === undefined)
        throw Error(`Type: "${typeutil.debugType(change.piece.type)}" is not expected to be in the game`);
    if (change.piece.index === -1)
        throw Error("Piece has not been allocated in organizedPieces");
 ...
 */
function capturePiece(gamefile, change) {
    if (change.action !== 'capture')
        throw new Error(`capturePiece called with a non-capture change: ${change.action}`);
 ...
 * Undos a capture
 */
function uncapturePiece(gamefile, change) {
    if (change.action !== 'capture')
        throw new Error(`uncapturePiece called with a non-capture change: ${change.action}`);
 ...
 */
function runChanges_Position(position, changes) {
    for (const change of changes) {
 ...
 */
function getCapturedPieceTypes(move) {
    const pieceTypes = new Set();
    move.changes.forEach(change => {
        if (change.action === 'capture')
            pieceTypes.add(change.capturedPiece.type);
    });
    return pieceTypes;
}
/**
