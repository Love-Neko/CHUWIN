...
self.onmessage = function (e) {
    ...
    if (!engineInitialized)
        initEvalWeightsAndSearchProperties(); // initialize the eval function weights and global search properties
...
let enginePositionCounter;
/** Time limit for the engine to think in milliseconds */
let engineTimeLimitPerMoveMillis;
...
let globallyBestScore;
let globalSurvivalPlies;
let globallyBestVariation;
...
function invertPieceNameDictionary(json) {
    const inv = {};
...
function get_center_of_mass(piece_type, cutoff, piecelist, coordlist) {
    let numpieces = 0;
    let center = [0, 0];
...
function rider_threatens(direction, piece_square, target_square, is_huygen, piecelist, coordlist, { exclude_white_piece_squares = false, ignore_blockers = false, threatening_own_square = false } = {}) {
    if (threatening_own_square && squares_are_equal(piece_square, target_square))
        return true;
    const [works, distance] = is_natural_multiple([target_square[0] - piece_square[0], target_square[1] - piece_square[1]], direction);
    if (!works)
        return false;
    if (is_huygen && !isprime.primalityTest(distance, null))
        return false;
    if (ignore_blockers)
        return true;
    ...
    for (let i = 0; i < coordlist.length; i++) {
        ...
        if (!collinear)
            continue;
        else if (is_huygen && !isprime.primalityTest(thispiecedistance, null))
            continue;
        else if (thispiecedistance < distance)
            return false;
    }
    return true;
}
...
function piece_threatens_square(piece_index, target_square, piecelist, coordlist) {
    ...
    if (squares_are_equal(piece_square, target_square))
        return false;
    // pawn threatening
    if (piece_properties.is_pawn) {
        if (squares_are_equal(add_move(piece_square, [-1, 1]), target_square) || squares_are_equal(add_move(piece_square, [1, 1]), target_square))
            return true;
        else
            return false;
    }
    ...
    if (piece_properties.jumps) {
        if (tuplelist_contains_tuple(piece_properties.jumps, [target_square[0] - piece_square[0], target_square[1] - piece_square[1]]))
            return true;
    }
    ...
    if (piece_properties.rides) {
        for (const ride_directrion of piece_properties.rides) {
            ...
            if (rider_threatens(ride_directrion, piece_square, target_square, is_huygen, piecelist, coordlist))
                return true;
        }
    }
    return false;
...
function square_is_threatened(target_square, piecelist, coordlist) {
    for (let index = 0; index < coordlist.length; index++) {
        if (piece_threatens_square(index, target_square, piecelist, coordlist))
            return true;
    }
    return false;
}
 ...
 */
function get_black_legal_moves(inTrapFleeMode, piecelist, coordlist) {
 ...
 */
function get_black_legal_move_amount(inTrapFleeMode, piecelist, coordlist) {
...
function isBlackInTrap(piecelist, coordlist) {
    let nearbyNonroyalWhites = 0;
    for (let i = 0; i < piecelist.length; i++) {
        if (piecelist[i] !== 0 && manhattanNorm(coordlist[i]) <= maxDistanceForTrap) {
            if (!pieceTypeDictionary[piecelist[i]].is_royal)
                nearbyNonroyalWhites++;
            // black is not in trap if white royal is nearby
            else if (manhattanNorm(coordlist[i]) <= maxDistanceForRoyal_Flee)
                return false;
        }
    }
...
function isBlackNearProtectedRider(piecelist, coordlist) {
    for (let i = 0; i < piecelist.length; i++) {
...
function get_white_piece_candidate_squares(piece_index, piecelist, coordlist) {
    ...
    const piece_properties = pieceTypeDictionary[piece_type];
    const piece_square = coordlist[piece_index];
    if (ignorepawnmoves && piece_properties.is_pawn)
        return candidate_squares;
    if (ignoreroyalmoves && piece_properties.is_royal)
        return candidate_squares;
    // jump moves
    if (piece_properties.jumps) {
        const num_jumps = piece_properties.jumps.length;
        const shortrangeLimit = shortRangeJumpDictionary[piece_type];
        let best_target_square;
        ...
        for (let move_index = 0; move_index < num_jumps; move_index++) {
            ...
            if (square_is_occupied(target_square, piecelist, coordlist))
                continue;
        ...
        if (candidate_squares.length === 0 && best_target_square !== undefined && (!piece_properties.rides || piece_properties.is_huygen))
    ...
    if (piece_properties.rides) {
        ...
        for (let i1 = 0; i1 < num_directions; i1++) {
            ...
            for (let i2 = i1 + 1; i2 < num_directions; i2++) {
                ...
                add_suitable_squares_to_candidate_list(candidate_squares, piece_index, piece_square, v1, v2, c1_min, c1_max, c2_min, c2_max, piecelist, coordlist);
                // adds suitable squares along v2 to the candidates list
                add_suitable_squares_to_candidate_list(candidate_squares, piece_index, piece_square, v2, v1, c2_min, c2_max, c1_min, c1_max, piecelist, coordlist);
            }
        }
    }
    return candidate_squares;
...
function add_suitable_squares_to_candidate_list(candidate_squares, piece_index, piece_square, v1, v2, c1_min, c1_max, c2_min, c2_max, piecelist, coordlist) {
    ...
    candidates_loop: for (let rc1 = c1_min; rc1 <= c1_max; rc1++) {
        ...
        if (tuplelist_contains_tuple(candidate_squares, target_square))
        ...
        if (is_huygen) {
            ...
            if (!isprime.primalityTest(distance, null))
                continue candidates_loop;
        }
        ...
        if (!rider_threatens(v1, piece_square, target_square, is_huygen, piecelist, coordlist, { exclude_white_piece_squares: true }))
            continue;
        ...
        // target square is not a royal move
            ...
            redundancy_loop: for (let i = 0; i < candidate_squares.length; i++) {
                ...
                else if (rider_threatens(v2, target_square, candidate_squares[i], is_huygen, piecelist, coordlist, { ignore_blockers: true }))
                    continue candidates_loop;
...
function get_white_candidate_moves(inProtectedRiderFleeMode, piecelist, coordlist) {
 ...
 * @returns {Number}
 */
function get_position_evaluation(piecelist, coordlist, black_to_move, inTrapFleeMode, inProtectedRiderFleeMode) {
    let score = 0;
    ...
    if (checkmateSelectedID in centerOfMassEvalDictionary) {
        ...
        if (center_of_mass)
            score += weight * distancefunction(center_of_mass);
    }
    return score;
}
/**
 ...
 */
function alphabeta(piecelist, coordlist, depth, start_depth, black_to_move, followingPrincipal, inTrapFleeMode, inProtectedRiderFleeMode, black_killer_list, white_killer_list, alpha, beta, alphaPlies, betaPlies) {
    ...
    if (black_to_move) {
        ...
        let black_killer_move = undefined;
        ...
        if (mayEnterProtectedRiderFleeMode && depth === start_depth && isBlackNearProtectedRider(piecelist, coordlist))
            inProtectedRiderFleeMode = true;
        // Order black moves by immediate evaluation function
        if (depth > 1 && black_moves.length > 1) {
            const black_move_evals = [];
        ...
        if (black_killer_list.length > 0) {
            const reordered_moves_killers = [];
            const reordered_moves_nonkillers = [];
            for (const move of black_moves) {
                if (tuplelist_contains_tuple(black_killer_list, move))
                    reordered_moves_killers.push(move); // Add killer moves to the first list
                else
                    reordered_moves_nonkillers.push(move); // Add non-killer moves to second list
            }
        ...
        let white_killer_piece_index = undefined;
        let candidate_moves;
        if (inTrapFleeMode && start_depth - depth > 1)
            candidate_moves = [[coordlist[0]], ...Array(piecelist.length - 1).fill([])];
        else
            candidate_moves = get_white_candidate_moves(inProtectedRiderFleeMode, piecelist, coordlist);
        ...
        if (white_killer_list.length > 0) {
            const reordered_indices_killers = [];
            const reordered_indices_nonkillers = [];
            for (const piece_index of indices) {
                if (piece_index in white_killer_list)
                    ...
                    reordered_indices_nonkillers.push(piece_index); // Add non-killer moves to second list
            }
            indices.length = 0;
        ...
        for (let depth = 1; depth <= maxdepth; depth = depth + 2) {
            ...
            if (depth === 1) {
                ...
                if (new_piecelist.filter(x => x === 0).length > piecelist.filter(x => x === 0).length) {
                    const piecesOrganizedByKey = new Map();
                    piecesOrganizedByKey.set("0,0", (royal_type === "k" ? r.KING + e.B : r.ROYALCENTAUR + e.B));
                    for (let i = 0; i < piecelist.length; i++) {
                        if (new_piecelist[i] !== 0) {
                            piecesOrganizedByKey.set(new_coordlist[i].toString(), invertedPieceNameDictionaty[new_piecelist[i]]);
                        }
                    }
                    ...
                    if (insufficientmaterial.detectInsufficientMaterial(dummy_gamefile))
                        break;
                }
...
function doesTypeExist(gamefile, type) {
    const range = gamefile.pieces.typeRanges.get(type);
    if (range === undefined)
        return false;
    return range.end - range.start - range.undefineds.length > 0;
}
function getFirstOfType(gamefile, type) {
    const range = gamefile.pieces.typeRanges.get(type);
    if (range === undefined)
        return;
    if (range.end - range.start - range.undefineds.length <= 0)
        return;
    let undefinedidx = 0;
    ...
    try {
        ...
        for (const [type, range] of input_gamefile.pieces.typeRanges) {
            ...
            for (let idx = range.start; idx < range.end; idx++) {
                ...
                if (Math.floor(type / numTypes) !== players.WHITE)
                    continue;
        ...
        let positionInBestMoveList = false;
