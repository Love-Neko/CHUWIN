 ...
 */
function sendMove() {
    if (!onlinegame.areInOnlineGame() || !onlinegame.areInSync() || !websocket.areSubbedToSub('game'))
        return; // Skip
    // console.log("Sending our move..");
    const gamefile = gameslot.getGamefile();
    const lastMove = moveutil.getLastMove(gamefile.moves);
    if (lastMove.isNull)
        throw Error('Cannot submit null move to online game.');
 ...
 */
function handleOpponentsMove(gamefile, message) {
    ...
    let moveDraft; // { startCoords, endCoords, promotion }
    try {
    ...
    if (moveIsLegal !== true)
        console.log(`Buddy made an illegal play: ${JSON.stringify(message.move)}. Move number: ${message.moveNumber}`);
    ...
    if (gamefile.mesh.offset)
        movesequence.animateMove(move, true); // ONLY ANIMATE if the mesh has been generated. This may happen if the engine moves extremely fast on turn 1.
