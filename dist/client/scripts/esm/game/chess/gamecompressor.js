 ...
 */
function compressGamefile(gamefile, copySinglePosition) {
    ...
    const movesCopy = jsutil.deepCopyObject(movepiece.ensureMovesNotNull(gamefile.moves));
    let positionString;
    let fullMove;
    if (gamefile.startSnapshot) {
        ...
        if (!gamefile.editor)
            throw Error("startSnapshot missing in non-editor mode");
        if (gamefile.moves.length > 0)
            throw Error("Should not be moves present in editor mode");
        if (copySinglePosition)
            throw Error('copySinglePosition has no effect in editor mode');
    ...
    if (gamestate.state_global.enpassant) { // In the form: { square: Coords, pawn: Coords },
        ...
        if (yDistance === 1)
            abridgedGamefile.enpassant = gamestate.state_global.enpassant.square; // Don't assign it if the distance is more than 1 square (not compatible with ICN)
        else
            console.warn("Enpassant distance is more than 1 square, not assigning it to the ICN. Enpassant:", gamestate.state_global.enpassant);
    }
 ...
 */
function GameToPosition(longform, moves, halfmoves = 0) {
    if (halfmoves === Infinity)
        halfmoves = moves.length; // If we want the final position, set halfmoves to the length of the moves array
    if (moves.length < halfmoves)
        throw Error(`Cannot convert game to position. Moves length (${moves.length}) is less than desired halfmoves (${halfmoves}).`);
    if (halfmoves === 0)
        return longform; // No changes needed
