...
function setBoardPos(newPos) {
    if (isNaN(newPos[0]) || isNaN(newPos[1]))
        return console.error(`Cannot set boardPos to ${newPos}!`);
    boardPos = [...newPos];
    frametracker.onVisualChange();
}
function setBoardScale(newScale) {
    if (isNaN(newScale))
        return console.error(`Cannot set scale to ${newScale}!`);
    if (newScale <= 0)
        return console.error(`Cannot set scale to ${newScale}!`);
    // Cap the scale
...
function setScaleVel(newScaleVel) {
    if (isNaN(newScaleVel))
        return console.error(`Cannot set scaleVel to ${newScaleVel}!`);
    if (Math.abs(newScaleVel) >= 100)
        console.warn(`Very large scaleVel: (${newScaleVel})`);
...
function update() {
    ...
    if (transition.areWeTeleporting())
        return; // Exit if we are teleporting
    if (loadbalancer.gisAFK())
        return; // Exit if we're AFK. Save our CPU!
    panBoard();
    recalcScale();
}
...
function panBoard() {
    if (panVel[0] === 0 && panVel[1] === 0)
        return; // Exit if we're not moving
...
function recalcScale() {
    if (scaleVel === 0)
        return; // Exit if we're not zooming
