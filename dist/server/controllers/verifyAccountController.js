...
async function verifyAccount(req, res) {
    ...
    if (!changesMade)
        return logEvents(`No changes made when saving verification for member "${username}" of id "${user_id}"! Value: ${JSON.stringify(verification)}`, 'errLog.txt', { print: true });
 ...
 */
function getNewVerificationAfterVerifying() {
    return {
        verified: true,
        notified: false,
    };
 ...
 */
function manuallyVerifyUser(usernameCaseInsensitive) {
    ...
    // The verification is stringified in the database. We need to parse it here.
    let verification = stringifiedVerificationOrNull === null ? null : JSON.parse(stringifiedVerificationOrNull); // { verified (boolean), notified (boolean), code (string) }
...
export { verifyAccount, manuallyVerifyUser, };
