...
/** What the AI-opponent panel is told as a turn progresses. */
type AIOpponentEvent =
	| { kind: 'thinking' }
	| { kind: 'moved', comment?: string, wasRandom: boolean, failure?: string, elapsedMillis?: number, upstreamAttempts?: number }
	| { kind: 'error', message: string };

/** Notified of AI-opponent progress. */
type AIOpponentListener = (event: AIOpponentEvent) => void;
 ...
 */
let aiListener: AIOpponentListener | undefined;
...
async function onMovePlayed() {
	...
	if (currentEngine === AI_OPPONENT_ENGINE) return await runAIOpponentTurn(gamefile);
 ...
 */
async function runAIOpponentTurn(gamefile: any): Promise<void> {
	if (aiThinking) return; // Already on it
	aiThinking = true;
	...
	try {
		...
		if (!inEngineGame || currentEngine !== AI_OPPONENT_ENGINE) return;
		if (gameslot.getGamefile() !== gamefile) return;
		if (gamefile.whosTurn !== engineColor || gamefile.gameConclusion) return;

		if (result === undefined) return; // No legal moves; the conclusion check will catch it
...
function isAIOpponentThinking(): boolean {
	return aiThinking;
}

/** True when this game's opponent is the configured LLM. */
function areInAIOpponentGame(): boolean {
 ...
 */
function retryAIOpponentMove(): void {
	if (!areInAIOpponentGame() || aiThinking) return;
	const gamefile = gameslot.getGamefile();
...
function setAIOpponentListener(listener: AIOpponentListener | undefined): void {
	aiListener = listener;
}
