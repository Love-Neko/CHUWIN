...
function addConnectionToConnectionLists(ws) {
    ...
    if (printIncomingAndClosingSockets)
        console.log(`New WebSocket connection established. Socket count: ${Object.keys(websocketConnections).length}. Metadata: ${socketUtility.stringifySocketMetadata(ws)}`);
}
/**
 ...
 */
function addConnectionToList(collection, key, id) {
    if (key === undefined)
        return; // No key, no operation
    if (!collection[key])
        collection[key] = []; // Initialize the array if it doesn't exist
    collection[key].push(id); // Add the socket ID to the list
}
function startTimerToExpireSocket(ws) {
 ...
 */
function removeConnectionFromConnectionLists(ws, code, reason) {
    ...
    if (printIncomingAndClosingSockets)
        console.log(`WebSocket connection has been closed. Code: ${code}. Reason: ${reason}. Socket count: ${Object.keys(websocketConnections).length}`);
}
/**
 ...
 */
function removeConnectionFromList(collection, key, id) {
    if (key === undefined || !collection[key])
        return; // No key or collection doesn't exist
...
function terminateAllIPSockets(IP) {
    ...
    if (connectionList === undefined)
        return; // IP is defined, but they don't have any sockets to terminate!
 ...
 */
function closeAllSocketsOfSession(jwt, closureCode, closureReason) {
 ...
 */
function closeAllSocketsOfMember(user_id, closureCode, closureReason) {
    const socketIDs = connectedMembers[user_id];
    if (!socketIDs)
        return; // This member doesn't have any connected sockets
 ...
 */
function doesClientHaveMaxSocketCount(IP) {
 ...
 * @returns *true* if they have too many sockets.
 */
function doesSessionHaveMaxSocketCount(jwt) {
    if (connectedSessions[jwt] === undefined)
        return false;
...
function unsubSocketFromAllSubs(ws, closureNotByChoice) {
    if (!ws.metadata.subscriptions)
        return; // No subscriptions
    const subscriptions = ws.metadata.subscriptions;
    const subscriptionsKeys = Object.keys(subscriptions);
    for (const key of subscriptionsKeys)
        handleUnsubbing(ws, key, closureNotByChoice);
}
// Set closureNotByChoice to true if you don't immediately want to disconnect them, but say after 5 seconds
function handleUnsubbing(ws, key, closureNotByChoice) {
...
export { addConnectionToConnectionLists, removeConnectionFromConnectionLists, terminateAllIPSockets, doesClientHaveMaxSocketCount, doesSessionHaveMaxSocketCount, generateUniqueIDForSocket, unsubSocketFromAllSubs, handleUnsubbing, closeAllSocketsOfSession, closeAllSocketsOfMember, };
