...
function updateCapturedPiece() {
    ...
    if (legalCaptureHoveredArrows.length === 1)
        capturedPieceThisFrame = legalCaptureHoveredArrows[0].piece;
}
function getCaptureCoords() {
...
function shiftArrows() {
    ...
    let newLocationOfSelectedPiece;
    if (capturedPieceThisFrame !== undefined) {
        // Reflect the dragged piece's new location in draganimation.ts
        const worldCoords = space.convertCoordToWorldSpace(capturedPieceThisFrame.coords);
