 ...
 */
let spritesheet; // Texture. Grid containing every texture of every piece, black and white.
/**
...
function getSpritesheet() {
    if (!spritesheet)
        throw new Error("Should not be getting the spritesheet when not loaded!");
    return spritesheet;
}
...
function getSpritesheetDataTexLocation(type) {
    if (!spritesheetData)
        throw new Error("Should not be getting texture locations when the spritesheet is not loaded!");
    if (!spritesheetData.texLocs[type])
        throw Error("No texture location for piece type: " + type);
...
async function initSpritesheetForGame(gl, gamefile) {
    ...
    // @ts-ignore
    const types = gamefile.existingTypes.filter(type => !typeutil.SVGLESS_TYPES.includes(typeutil.getRawType(type)));
    // Convert each SVG element to an Image
    const readyImages = types.map(t => imagecache.getPieceImage(t));
