 ...
 */
function handleServerGameUpdate(gamefile, message) {
    ...
    if (result.opponentPlayedIllegalMove)
        return;
...
function synchronizeMovesList(gamefile, moves, claimedGameConclusion) {
    ...
    while (i < moves.length - 1) { // Increment i, adding the server's correct moves to our moves list
        ...
        if (opponentPlayedThisMove) { // Perform legality checks
            ...
            if (moveIsLegal !== true)
                console.log(`Buddy made an illegal play: ${thisShortmove} ${claimedGameConclusion}`);
        ...
        if (isLastMove)
            movesequence.animateMove(move, true); // Only animate on the last forwarded move.
        console.log("Forwarded one move while resyncing to online game.");
        aChangeWasMade = true;
    }
    if (!aChangeWasMade)
        movesequence.viewIndex(gamefile, originalMoveIndex);
    else
