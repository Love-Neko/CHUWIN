...
/** The far-right button. Opens/closes the AI coach panel. */
const element_aicoach = document.getElementById('nav-aicoach');
...
function toggle() {
    if (navigationOpen)
        ...
        open({ allowEditCoords: !onlinegame.areInOnlineGame() });
...
function updateElement_Coords() {
    if (isCoordinateActive())
        return; // Don't update the coordinates if the user is editing them
...
function initListeners_Navigation() {
    ...
    element_aicoach.addEventListener('click', callback_AICoach);
...
function closeListeners_Navigation() {
    ...
    element_aicoach.removeEventListener('click', callback_AICoach);
...
function recenter() {
    ...
    if (!boundingBox)
        return console.error("Cannot recenter when the bounding box of the starting position is undefined!");
...
function update_MoveButtons() {
    ...
    if (decrementingLegal)
        element_moveRewind.classList.remove('opacity-0_5');
    else
        element_moveRewind.classList.add('opacity-0_5');
    if (incrementingLegal)
        element_moveForward.classList.remove('opacity-0_5');
    else
        element_moveForward.classList.add('opacity-0_5');
}
function callback_Pause() {
    guipause.open();
}
/** The coach panel is its own toggle, so a second click closes it again. */
function callback_AICoach() {
    guiaicoach.toggle();
}
// Mouse
...
function testIfRewindMove() {
    if (!listener_document.isKeyDown('ArrowLeft'))
        return;
    if (rewindIsLocked)
        return;
    rewindMove();
...
function testIfForwardMove() {
    if (!listener_document.isKeyDown('ArrowRight'))
        return;
    forwardMove();
}
...
function rewindMove() {
    const gamefile = gameslot.getGamefile();
    if (!moveutil.isDecrementingLegal(gamefile))
        return stats.showMoves();
    frametracker.onVisualChange();
...
function forwardMove() {
    const gamefile = gameslot.getGamefile();
    if (!moveutil.isIncrementingLegal(gamefile))
        return stats.showMoves();
