...
const element_showAccountInfo = document.getElementById('show-account-info');
...
const member = docutil.getLastSegmentOfURL(); // Assuming returns string
// --- Initialization ---
    ...
    try {
        ...
        if (loggedInAs && loggedInAs === result.username) {
            ...
            if (result.verified === true)
                element_verifyConfirmElement.classList.remove('hidden'); // They just confirmed, tell them it was a success!
...
function updateCompletedCheckmatesInformation(checkmates_beaten) {
    ...
    if (numCompleted >= numTotal)
        shownBadge = element_checkmateBadgeGold;
    else if (numCompleted >= 0.75 * numTotal)
        shownBadge = element_checkmateBadgeSilver;
    else if (numCompleted >= 0.5 * numTotal)
        shownBadge = element_checkmateBadgeBronze;
...
async function removeAccount(confirmation) {
    if (confirmation) {
        if (!confirm(translations["js-confirm_delete"]))
    ...
    if (cancelWasPressed)
        return; // User pressed Cancel in the password prompt
...
function resendConfirmEmail() {
    if (!isOurProfile)
        return; // Only allow resend if viewing own profile
    const config = {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
...
function recalcUsernameSize() {
    ...
    if (!usernameText)
        return; // Exit if no username text
