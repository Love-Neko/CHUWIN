 ...
 */
function removeCheckInvalidMoves(gamefile, moves, pieceSelected, color) {
    if (color === players.NEUTRAL)
        return; // Neutral pieces can't be in check
    if (!gamefileutility.isOpponentUsingWinCondition(gamefile, color, 'checkmate'))
        return;
 ...
 */
function removeCheckInvalidMoves_Individual(gamefile, individualMoves, piece, color) {
    ...
    for (let i = individualMoves.length - 1; i >= 0; i--) { // Iterate backwards so we don't run into issues as we delete indices while iterating
        ...
        if (isMoveCheckInvalid(gamefile, piece, thisMove, color))
            individualMoves.splice(i, 1); // Remove the move
    }
}
/**
 ...
 * @param color - The color of the player the piece belongs to.
 */
function removeCheckInvalidMoves_Sliding(gamefile, moves, piece, color) {
    if (!moves.sliding)
        return; // No sliding moves to remove
    /** List of coordinates of all our royal jumping pieces */
    const royalCoords = boardutil.getJumpingRoyalCoordsOfColor(gamefile.pieces, color);
    if (royalCoords.length === 0)
        return; // No royals, no open discoveries, don't remove any sliding moves
    ...
    if (addressExistingChecks(gamefile, moves, royalsInCheck, piece.coords, color))
        return;
    /**
...
function addressExistingChecks(gamefile, legalMoves, royalCoords, selectedPieceCoords, color) {
    if (royalCoords.length === 0)
        return false; // Exit if nothing in check
    if (!checkdetection.isPlayerInCheck(gamefile, color))
        return false; // Our OPPONENT is in check, not us! Them being in check doesn't restrict our movement!
    ...
    let capturingMove; // We will ONLY add this move if all sliding moves are deleted, otherwise it may be a duplicate.
    ...
    if (!attacker.slidingCheck && (attacker.path?.length ?? 2) < 3
        ...
        if (capturingMove)
            legalMoves.individual.push(capturingMove); // Add this, now that we know all sliding moves were deleted.
        return true;
    }
    /**
    ...
    if (attacker.slidingCheck)
        appendBlockingMoves(gamefile, royalCoords[0], attacker.coords, legalMoves, selectedPieceCoords, color); // Has a chance to delete all sliding moves except one, adding the `brute` flag.
    else
        appendPathBlockingMoves(gamefile, attacker.path, legalMoves, selectedPieceCoords, color);
    if (!legalMoves.brute) {
 ...
 * @param color - The color of the player the piece belongs to.
 */
function removeSlidingMovesThatOpenDiscovered(gamefile, moves, pieceSelected, color) {
    if (checkdetection.isPlayerInCheck(gamefile, color))
    ...
    outer: if (checkResults.check) {
        ...
        for (const checkedRoyalCoords of checkResults.royalsInCheck) {
            for (const attacker of checkResults.attackers) {
                ...
                if (!math.areLinesInGeneralFormEqual(line1GeneralForm, line2GeneralForm))
                ...
                for (const slideDir of Object.keys(moves.sliding)) { // 'dx,dy'
                    const slideDirVec = math.getVec2FromKey(slideDir); // [dx,dy]
                    ...
                    if (!math.areLinesInGeneralFormEqual(line1GeneralForm, slideLineGeneralForm))
                        delete moves.sliding[slideDir]; // Not the same line, delete it.
                }
            }
        }
        if (Object.keys(moves.sliding).length === 0)
            delete moves.sliding; // No sliding moves left
        // For any slides left, if colinears exist in the game, flag the legal moves to brute force check each square for check
        else if (gamefile.colinearsPresent)
            moves.brute = true;
    }
 ...
 * @param color - The color of friendlies
 */
function appendBlockingMoves(gamefile, square1, square2, moves, coords, color) {
    ...
    for (const lineKey in moves.sliding) { // 'dx,dy'
        ...
        if (blockPoint === undefined && gamefile.colinearsPresent && math.areLinesInGeneralFormEqual(line1GeneralForm, line2GeneralForm)) {
            ...
            for (const slideDir in moves.sliding) { // 'dx,dy'
                if (slideDir === lineKey)
                    continue; // Same line, don't delete this one.
                // Different line... but is it colinear? If so we also want to keep it.
                const thisSlideDir = coordutil.getCoordsFromKey(slideDir); // [dx,dy]
                const thisLineGeneralForm = math.getLineGeneralFormFromCoordsAndVec(coords, thisSlideDir);
                if (!math.areLinesInGeneralFormEqual(line1GeneralForm, thisLineGeneralForm))
                    delete moves.sliding[slideDir]; // Not colinear, delete it.
            }
            break; // All other slides were deleted, no point in continuing to iterate.
        }
        if (blockPoint === undefined)
            continue; // None (or infinite) intersection points!
        if (!math.boxContainsSquare(box, blockPoint))
            continue; // Intersection point not between our 2 points, but outside of them.
        if (!coordutil.areCoordsIntegers(blockPoint))
            continue; // It doesn't intersect at a whole number, impossible for our piece to move here!
        if (coordutil.areCoordsEqual(blockPoint, square1))
            continue; // Can't move onto our piece that's in check..
        if (coordutil.areCoordsEqual(blockPoint, square2))
            continue; // nor to the piece that is checking us (those are added prior to this if it's legal)!
        ...
        if (gamefile.colinearsPresent && moves.individual.some((move) => move[0] === blockPoint[0] && move[1] === blockPoint[1]))
            continue;
        // Can our piece legally move there?
        if (legalmoves.checkIfMoveLegal(gamefile, moves, coords, blockPoint, color, { ignoreIndividualMoves: true }))
            moves.individual.push(blockPoint); // Can block!
    }
}
/**
 ...
 * @param color - The color of friendlies
 */
function appendPathBlockingMoves(gamefile, path, legalMoves, selectedPieceCoords, color) {
    /**
 ...
 */
function isMoveCheckInvalid(gamefile, piece, destCoords, color) {
    const moveDraft = { startCoords: jsutil.deepCopyObject(piece.coords), endCoords: moveutil.stripSpecialMoveTagsFromCoords(destCoords) };
 ...
 */
function getSimulatedCheck(gamefile, moveDraft, colorToTestInCheck) {
    return movepiece.simulateMoveWrapper(gamefile, moveDraft, () => checkdetection.detectCheck(gamefile, colorToTestInCheck));
}
// Exports --------------------------------------------------------------------------------
export default {
