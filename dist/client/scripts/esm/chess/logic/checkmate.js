 ...
 */
function detectCheckmateOrStalemate(gamefile) {
    ...
    for (const rType of Object.values(rawTypes)) {
        ...
        if (!thesePieces)
            continue; // The game doesn't have this type of piece
        for (let idx = thesePieces.start; idx < thesePieces.end; idx++) {
            const thisPiece = boardutil.getPieceFromIdx(gamefile.pieces, idx);
            if (!thisPiece)
                continue; // Piece undefined. We leave in deleted pieces so others retain their index!
    ...
    if (gamefileutility.isCurrentViewedPositionInCheck(gamefile) && usingCheckmate) {
        ...
        return `${players.NEUTRAL} stalemate`; // Victor of player NEUTRAL means it was a draw.
}
