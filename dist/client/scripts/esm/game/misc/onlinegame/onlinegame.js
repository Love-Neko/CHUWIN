...
function areWeColorInOnlineGame(color) {
    if (!inOnlineGame)
        return false; // Can't be that color, because we aren't even in a game.
 ...
 * has been called, which IS ONLY called once the SERVER tells us the result of the game, not us!
 */
function hasServerConcludedGame() {
    if (!inOnlineGame)
        throw Error("Cannot get serverHasConcludedGame of online game when we're not in an online game.");
...
function set_DrawOffers_DisconnectInfo_AutoAFKResign_ServerRestarting(options) {
    ...
    if (options.disconnect)
        disconnect.startOpponentDisconnectCountdown(options.disconnect);
    else
        disconnect.stopOpponentDisconnectCountdown();
    // If Opponent is currently afk, display that countdown
    if (options.millisUntilAutoAFKResign !== undefined)
        afk.startOpponentAFKCountdown(options.millisUntilAutoAFKResign);
    else
        afk.stopOpponentAFKCountdown();
    // If the server is restarting, start displaying that info.
    if (options.serverRestartingAt !== undefined)
        serverrestart.initServerRestart(options.serverRestartingAt);
    else
        serverrestart.resetServerRestarting();
}
...
function confirmNavigationAwayFromGame(event) {
    ...
    if (event.metaKey || event.ctrlKey)
        return; // Allow opening in a new tab without confirmation
    if (gamefileutility.isGameOver(gameslot.getGamefile()))
        return;
    const userConfirmed = confirm('Are you sure you want to leave the game?');
    if (userConfirmed)
        return; // Follow link like normal. Server then starts a 20-second auto-resign timer for disconnecting on purpose.
...
function onMainMenuPress() {
    ...
    if (serverHasConcludedGame)
        return; // Don't need to abort/resign, game is already over
...
function onGameConclude() {
    if (!inOnlineGame)
        return; // The game concluded wasn't an online game.
 ...
 */
function adjustClockValuesForPing(clockValues) {
    if (!clockValues.colorTicking)
        return clockValues; // No clock is ticking (< 2 moves, or game is over), don't adjust for ping
    ...
    if (halfPing > 2500)
        console.error("Ping is above 5000 milliseconds!!! This is a lot to adjust the clock values!");
