...
function fourDimensionalPawnMove(gamefile, coords, color) {
    const legalMoves = [];
 ...
 */
function pawnLegalMoves(gamefile, coords, color, movetype) {
    ...
    const coordsInFront = [coords[0], coords[1] + yDistanceParity];
    if (!boardutil.isPieceOnCoords(gamefile.pieces, coordsInFront)
        ...
        const doublePushCoord = [coordsInFront[0], coordsInFront[1] + yDistanceParity];
    ...
    if (strong_pawns)
        coordsToCapture.push(// Add the brawn-like captures
        [coords[0] - distance_complement, coords[1] + yDistanceParity], [coords[0] + distance_complement, coords[1] + yDistanceParity]);
    for (let i = 0; i < coordsToCapture.length; i++) {
        ...
        if (rawTypeOfPiece === r.VOID || color === colorOfPiece)
            continue; // Same color, don't add the capture
    ...
    if (strong_pawns)
        addPossibleEnPassant(gamefile, individualMoves, coords, color, distance_complement, distance);
 ...
 * @param ydistance
 */
function addPossibleEnPassant(gamefile, individualMoves, coords, color, xdistance, ydistance) {
    if (!gamefile.state.global.enpassant)
        return; // No enpassant flag on the game, no enpassant possible
    if (color !== gamefile.whosTurn)
        return; // Not our turn (the only color who can legally capture enpassant is whos turn it is). If it IS our turn, this also guarantees the captured pawn will be an enemy pawn.
    ...
    if (capturedColor === color || capturedType === r.VOID)
        return; // The captured pawn is not an enemy pawn. THIS IS ONLY EVER NEEDED if we can move opponent pieces on our turn, which is the case in EDIT MODE.
    const xDifference = gamefile.state.global.enpassant.square[0] - coords[0];
    if (Math.abs(xDifference) !== xdistance)
        return; // Not immediately left or right of us
    const yDistanceParity = (color === players.WHITE ? ydistance : -ydistance);
    if (coords[1] + yDistanceParity !== gamefile.state.global.enpassant.square[1])
        return; // Not one in front of us
...
function appendPawnMoveAndAttachPromoteFlag(gamefile, individualMoves, landCoords, color) {
    if (gamefile.gameRules.promotionRanks !== undefined) {
        ...
        if (teamPromotionRanks.includes(landCoords[1]))
            landCoords.promoteTrigger = true;
    }
...
function doFourDimensionalPawnMove(gamefile, piece, move) {
    ...
    if (move.enpassantCreate !== undefined)
        state.createEnPassantState(move, gamefile.state.global.enpassant, move.enpassantCreate);
    if (!move.enpassant && !move.promotion)
        return false; // No special move to execute, return false to signify we didn't move the piece.
    ...
    if (capturedPiece !== undefined)
        boardchanges.queueCapture(moveChanges, true, piece, move.endCoords, capturedPiece); // Delete the piece captured
    else
 ...
 */
function fourDimensionalKnightMove(gamefile, coords, color) {
    const individualMoves = [];
    const dim = fourdimensionalgenerator.get4DBoardDimensions();
            ...
            for (let offsetH = 2; offsetH >= -2; offsetH--) {
                for (let offsetV = 2; offsetV >= -2; offsetV--) {
                    ...
                    if (baseH * baseH + baseV * baseV + offsetH * offsetH + offsetV * offsetV === 5) {
                        ...
                        if (endPiece !== undefined && (color === typeutil.getColorFromType(endPiece) || typeutil.getRawType(endPiece) === r.VOID))
                            continue;
                        // do not allow knight to leave the 4D board
                        if (endCoords[0] <= dim.MIN_X || endCoords[0] >= dim.MAX_X || endCoords[1] <= dim.MIN_Y || endCoords[1] >= dim.MAX_Y)
                            continue;
...
function fourDimensionalKingMove(gamefile, coords, color) {
    const legalMoves = kingLegalMoves(gamefile, coords, color);
 ...
 */
function kingLegalMoves(gamefile, coords, color) {
    const individualMoves = [];
    const dim = fourdimensionalgenerator.get4DBoardDimensions();
        ...
        for (let baseV = 1; baseV >= -1; baseV--) {
            for (let offsetH = 1; offsetH >= -1; offsetH--) {
                for (let offsetV = 1; offsetV >= -1; offsetV--) {
                    ...
                    if (!fourdimensionalgenerator.getMovementType().STRONG_KINGS_AND_QUEENS && baseH * baseH + baseV * baseV + offsetH * offsetH + offsetV * offsetV > 2)
                        continue;
                    if (baseH === 0 && baseV === 0 && offsetH === 0 && offsetV === 0)
                        continue;
                    const x = coords[0] + dim.BOARD_SPACING * baseH + offsetH;
                    const y = coords[1] + dim.BOARD_SPACING * baseV + offsetV;
                    const endCoords = [x, y];
