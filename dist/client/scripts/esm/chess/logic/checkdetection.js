 ...
 */
function detectCheck(gamefile, color, trackAttackers) {
    // Coordinates of ALL royals of this color!
    const royalCoords = boardutil.getRoyalCoordsOfColor(gamefile.pieces, color);
    // Array of coordinates of royal pieces that are in check
    const royalsInCheck = [];
    const attackers = trackAttackers ? [] : undefined;
...
function isSquareBeingAttacked(gamefile, coord, colorOfFriendly, attackers) {
    ...
    if (doesVicinityAttackSquare(gamefile, coord, colorOfFriendly, attackers)) {
        if (attackers)
            ...
            return true; // Not keeping track of attackers, exit early
    }
 ...
 */
function doesVicinityAttackSquare(gamefile, square, friendlyColor, attackers) {
    for (const [coordsKey, thisVicinity] of Object.entries(gamefile.vicinity)) {
        ...
        const actualSquare = [square[0] - thisSquare[0], square[1] - thisSquare[1]];
        ...
        if (typeOnSquare === undefined)
            continue; // Nothing there to capture us
        // Is it the same color?
        const [trimmedTypeOnSquare, typeOnSquareColor] = typeutil.splitType(typeOnSquare);
        if (friendlyColor === typeOnSquareColor)
            continue; // A friendly can't capture us
        if (typeOnSquareColor === players.NEUTRAL)
            continue; // Neutrals can't capture us either (GARGOYLE ALERT)
 ...
 */
function doesSpecialAttackSquare(gamefile, square, friendlyColor, attackers) {
    for (const [coordsKey, thisVicinity] of Object.entries(gamefile.specialVicinity)) {
        ...
        if (thisVicinity.includes(trimmedTypeOnSquare)) { // This square can POTENTIALLY be captured via special move...
            ...
            if (!legalmoves.checkIfMoveLegal(gamefile, specialPiecesLegalMoves, actualSquare, square, friendlyColor))
            ...
            if (attackers) {
                 ...
                 */
                if (square.path !== undefined)
                    attacker.path = square.path;
 ...
 */
function doesSlideAttackSquare(gamefile, square, friendlyColor, attackers) {
    let atleast1Attacker = false;
...
function doesLineAttackSquare(gamefile, line, direction, coords, color, attackers) {
    if (!line)
        return false; // This line doesn't exist, then obviously no pieces can attack our square
    ...
    for (const thisPieceIdx of line) { // { coords, type }
        ...
        if (color === thisPieceColor)
            continue; // Same team, can't capture us, CONTINUE to next piece!
        if (thisPieceColor === players.NEUTRAL)
            continue; // Neutrals can't move, that means they can't make captures, right?
        const thisPieceMoveset = legalmoves.getPieceMoveset(gamefile, thisPiece.type);
        if (!thisPieceMoveset.sliding)
            continue; // Piece has no sliding movesets.
        const moveset = thisPieceMoveset.sliding[directionKey];
        if (!moveset)
            continue; // Piece can't slide in the direction our line is going
        ...
        if (!thisPieceLegalSlide)
            continue; // This piece can't move in the direction of this line, NEXT piece!
...
function appendAttackerToList(attackers, attacker) {
    for (let i = 0; i < attackers.length; i++) {
        ...
        if (!coordutil.areCoordsEqual(thisAttacker.coords, attacker.coords))
            continue; // Not the same piece
        // The same piece...
        // Upgrade the slidingCheck to true, if applicable.
        if (attacker.slidingCheck)
            thisAttacker.slidingCheck = true;
        return;
    }
 ...
 */
function isPlayerInCheck(gamefile, color) {
