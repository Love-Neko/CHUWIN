...
function detectRepetitionDraw(gamefile) {
    ...
    const startIndex = moveList.length - 1;
    let indexOfLastEqualPositionFound = startIndex + 1; // We need +1 because the first move we observe is the move that brought us to this move index.
    outer: for (let index = startIndex; index >= 0; index--) { // WILL BE -1 if we've reached the beginning of the game!
        ...
        if (typeutil.getRawType(move.type) === rawTypes.PAWN)
            break; // Pawn pushes reset the repetition alg because we know they can't move back to their previous position.
        if (move.state.global.some((stateChange) => stateChange.type === 'specialrights' && stateChange.future === false))
        ...
        for (let i = 0; i < move.changes.length; i++) {
            ...
            if (boardchanges.oneWayActions.includes(change.action))
                break outer; // One-way action, can't be undone, no further equal positions.
        ...
        move.state.global.forEach((state) => {
            if (state.type !== 'enpassant')
                return false; // Filter out non-enpassant states
            /**
            ...
            if (state.future !== undefined)
                addDeficit(`${state.future.square[0]},${state.future.square[1]},${state.future.pawn[0]},${state.future.pawn[1]},enpassant`);
            if (state.current !== undefined)
                addSurplus(`${state.current.square[0]},${state.current.square[1]},${state.current.pawn[0]},${state.current.pawn[1]},enpassant`);
            return; // typescript needs this to not complain
        });
        function addSurplus(flux) {
        ...
        checkEqualPosition: {
            ...
            if (indexDiff < turnOrderLength)
                break checkEqualPosition; // Hasn't been a full turn cycle yet, don't increment the counter
            ...
            if (surplus.size === 0 && deficit.size === 0) {
                ...
                if (thisPlayerTurn !== currentPlayerTurn)
                    break checkEqualPosition;
                // Equal!
                equalPositionsFound++;
                indexOfLastEqualPositionFound = index;
                if (equalPositionsFound === 2)
                    break; // Enough to confirm a repetition draw!
            }
        }
