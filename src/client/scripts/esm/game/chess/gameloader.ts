...
/** The options `startLocalGame` takes. */
...
/** The options `startEngineGame` takes. */
interface EngineGameOptions {
	...
	youAreColor: Player,
	currentEngine: 'engineCheckmatePractice' | 'classicEngine' | 'aiOpponent', // Add more union types when more engines are added
	engineConfig: EngineConfig,
	/** What to call the opponent in the game's metadata. Defaults to "Engine". */
	opponentName?: string,
 ...
 */
let lastGameOptions: undefined | { kind: 'local', options: LocalGameOptions } | { kind: 'engine', options: EngineGameOptions };
...
function canReplayLastGame(): boolean {
	return lastGameOptions !== undefined;
}

/** Whether the last started game was against the LLM, which owns the chat panel. */
function wasLastGameVsLLM(): boolean {
	if (lastGameOptions === undefined || lastGameOptions.kind !== 'engine') return false;
 ...
 */
async function replayLastGame(): Promise<void> {
	const last = lastGameOptions;
	if (last === undefined) return;

	unloadGame(); // A new gamefile can't be loaded on top of the old one

	if (last.kind === 'local') await startLocalGame(last.options);
	else await startEngineGame(last.options);
}
...
async function startLocalGame(options: LocalGameOptions) {
...
async function startEngineGame(options: EngineGameOptions) {
