 ...
 */
function generateCubicSplineCoefficients(points) {
 ...
 */
function evaluateSplineAt(t, coefficients) {
 ...
 */
function generateSplinePath(controlPoints, resolution) {
    if (controlPoints.length < 3)
        return controlPoints; // A straight line already has infinite precision
    ...
    for (let i = 0; i < totalSegments; i++) {
        ...
        for (let k = 0; k <= resolution; k++) {
             ...
             */
            if (isLast && k === resolution)
                [x, y] = controlPoints[controlPoints.length - 1];
            path.push([x, y]);
        }
    }
    return path;
 ...
 */
function renderSplineDebug(controlPoints, width, color) {
    if (controlPoints.length < 2)
        throw Error("Spline requires at least 2 waypoints to render.");
    const vertexData = [];
