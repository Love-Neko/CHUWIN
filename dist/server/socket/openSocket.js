...
// Variables ---------------------------------------------------------------------------
// Functions ---------------------------------------------------------------------------
function onConnectionRequest(socket, req) {
    const ws = closeIfInvalidAndAddMetadata(socket, req);
    if (ws === undefined)
        return; // We will have already closed the socket
    // Rate Limit Here
...
function closeIfInvalidAndAddMetadata(socket, req) {
    ...
    const ws = socket; // Cast WebSocket to CustomWebSocket
    ws.metadata = {
 ...
 */
function addListenersToSocket(req, ws) {
