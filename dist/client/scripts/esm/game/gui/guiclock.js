...
function update(gamefile) {
    if (gamefile.untimed || gamefile.gameConclusion || !moveutil.isGameResignable(gamefile))
        return;
...
function edit(gamefile) {
    ...
    for (const [playerStr, clockElements] of Object.entries(element_timers)) {
        ...
        if (player === gamefile.clocks.colorTicking)
            continue;
        removeBorder(clockElements.timer);
    }
...
function rescheduleSoundEffects(gamefile) {
    if (gamefile.untimed || !moveutil.isGameResignable(gamefile) || gamefileutility.isGameOver(gamefile) || gamefile.clocks.timeRemainAtTurnStart === undefined)
        return; // Don't plenty of sound if the game is over several clock values are reset when the game ends.
...
function rescheduleMinuteTick(gamefile) {
    if (gamefile.clocks.startTime.minutes < lowtimeNotif.clockMinsRequiredToUse)
        return; // 1 minute lowtime notif is not used in bullet games.
    clearTimeout(lowtimeNotif.timeoutID);
    if (onlinegame.areInOnlineGame() && gamefile.clocks.colorTicking !== onlinegame.getOurColor())
        return; // Don't play the sound effect for our opponent.
    if (lowtimeNotif.playersNotified.has(gamefile.clocks.colorTicking))
        return;
...
function rescheduleTicking(gamefile) {
    ...
    if (onlinegame.areInOnlineGame() && gamefile.clocks.colorTicking !== onlinegame.getOurColor())
        return; // Don't play the sound effect for our opponent.
    if (gamefile.clocks.timeAtTurnStart < 10000)
        return;
    // We have to use this instead of reading the current clock values
    // because those aren't updated every frame when the page isn't focused!!
    const playerTrueTimeRemaining = clock.getColorTickingTrueTimeRemaining(gamefile);
...
function rescheduleTick(gamefile) {
    ...
    if (timeRemain > 0)
        countdown.tick.timeoutID = setTimeout(() => playTickEffect(0), timeRemain);
    else {
        const offset = -timeRemain;
...
function playDrumAndQueueNext(gamefile, secsRemaining) {
    if (secsRemaining === undefined)
        return console.error("Cannot play drum without secsRemaining");
    sound.playSound_drum();
    // We have to use this instead of reading the current clock values
    // because those aren't updated every frame when the page isn't focused!!
    const playerTrueTimeRemaining = clock.getColorTickingTrueTimeRemaining(gamefile);
    if (playerTrueTimeRemaining < 1500)
        return;
    // Schedule next drum...
    const newSecsRemaining = secsRemaining - 1;
    if (newSecsRemaining === 0)
        return; // Stop
