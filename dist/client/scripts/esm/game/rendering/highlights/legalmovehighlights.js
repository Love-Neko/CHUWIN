...
function onPieceSelected(piece, legalMoves) {
    pieceSelected = piece;
...
function isRenderRangeBoundingBoxOutOfRange() {
    if (!boundingBoxOfRenderRange)
        return true; // It isn't even initiated yet 
    ...
    if (width * multiplier * multiplier < renderRangeWidth && !perspective.getEnabled())
        return true;
 ...
 */
function generateModelsForPiecesLegalMoveHighlights(coords, legalMoves, friendlyColor, highlightColor) {
    ...
    const vertexData_NonCapture = usingDots ? legalmoveshapes.getDataLegalMoveDot(highlightColor) : legalmoveshapes.getDataLegalMoveSquare(highlightColor);
    ...
    const vertexData_Capture = usingDots ? legalmoveshapes.getDataLegalMoveCornerTris(highlightColor) : legalmoveshapes.getDataLegalMoveSquare(highlightColor);
 ...
 */
function concatData_HighlightedMoves_Individual(instanceData_NonCapture, instanceData_Capture, legalMoves, gamefile) {
    // Get an array of the list of individual legal squares the current selected piece can move to
    const legalIndividuals = legalMoves.individual;
    if (!legalIndividuals)
        return; // This piece doesn't have any legal jumping/individual moves.
    ...
    for (const coord of legalIndividuals) {
        ...
        if (isPieceOnCoords)
            instanceData_Capture.push(...offsetCoord);
        else
            instanceData_NonCapture.push(...offsetCoord);
    }
}
/**
 ...
 * @param friendlyColor - The color of friendly pieces
 */
function concatData_HighlightedMoves_Sliding(instanceData_NonCapture, instanceData_Capture, coords, legalMoves, gamefile, friendlyColor) {
    if (!legalMoves.sliding)
        return; // No sliding moves
    ...
    for (const lineKey of slideLines) { // '1,0'
        ...
        if (!intsect1Tile || !intsect2Tile)
            continue; // If there's no intersection point, it's off the screen, or directly intersect the corner, don't bother rendering.
 ...
 */
function concatData_HighlightedMoves_Diagonal(instanceData_NonCapture, instanceData_Capture, coords, step, intsect1Tile, intsect2Tile, limits, ignoreFunc, gamefile, friendlyColor, brute) {
    // Right moveset
 ...
 * @param brute - If true, each move will be simulated as to whether it results in check, and if so, not added to the mesh data.
 */
function concatData_HighlightedMoves_Diagonal_Split(instanceData_NonCapture, instanceData_Capture, coords, step, intsect1Tile, intsect2Tile, limit, ignoreFunc, gamefile, friendlyColor, brute) {
    if (limit === 0)
        return; // Quick exit
    ...
    const firstInstancePositionOffset = coordutil.subtractCoordinates(startCoords, model_Offset);
    ...
    if (xyDist < 0)
        return; // Early exit. The piece is up-right of our screen
 ...
 * @param brute - If true, each move will be simulated as to whether it results in check, and if so, not added to the mesh data.
 */
function addDataDiagonalVariant(instanceData_NonCapture, instanceData_Capture, firstInstancePositionOffset, step, iterateCount, startCoords, pieceCoords, ignoreFunc, gamefile, friendlyColor, brute) {
    for (let i = 0; i < iterateCount; i++) {
        ...
        legal: if (ignoreFunc(pieceCoords, thisCoord)) { // Ignore function PASSED. This move is LEGAL
            ...
            if (brute) {
                const moveDraft = { startCoords: pieceCoords, endCoords: thisCoord };
            ...
            if (isPieceOnCoords)
                instanceData_Capture.push(...firstInstancePositionOffset);
            else
                instanceData_NonCapture.push(...firstInstancePositionOffset);
        }
...
function renderOutlineofRenderBox() {
    if (!camera.getDebug())
        return; // Skip if camera debug mode off
    const color = [1, 0, 1, 1];
