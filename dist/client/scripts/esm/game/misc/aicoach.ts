...
import type { MoveDraft, CoordsSpecial } from '../../chess/logic/movepiece.js';
...
/** Which request/response shape the configured endpoint speaks. */
type Protocol = 'openai' | 'anthropic';

/** How hard a reasoning model is asked to think. `''` leaves the parameter off entirely. */
type ReasoningEffort = '' | 'minimal' | 'low' | 'medium' | 'high';

/** Everything the user fills into the AI Coach settings form. */
interface AICoachConfig {
	protocol: Protocol;
	/** Whatever the user pasted. The server normalizes it into a real endpoint. */
	baseUrl: string;
	apiKey: string;
	model: string;
	/**
	 * Optional separate model for the AI opponent's own moves. Blank falls back to
	 * {@link AICoachConfig.model}. Point this at something fast: picking a move is
	 * latency-critical in a way that coaching and chat aren't.
	 */
	moveModel: string;
	/**
	 * Sent as `reasoning_effort` on the openai protocol. The single biggest latency
	 * lever on reasoning models. Blank omits it, for endpoints that reject it.
	 */
	reasoningEffort: ReasoningEffort;
	temperature: number;
	maxTokens: number;
	/** Re-analyze automatically whenever it becomes our turn. */
	autoAnalyze: boolean;
	/**
	 * Let the coach comment on the player's own moves in the chat, unprompted.
	 * On by default: a coach that only speaks when asked isn't watching the game.
	 * It does spend the user's API key once per round, which is why it can be turned off.
	 */
	autoCritique: boolean;
}

/** A move the model proposed. */
...
/** The parsed result of one analysis. */
interface Advice {
	assessment: string;
	threats: string[];
	reasoning: string;
	...
	/** The model's untouched reply. Shown as-is when we couldn't parse JSON out of it. */
	raw: string;
	/** Set when we had to drop `bestMove` because it wasn't a legal move. */
	rejectedMove?: string;
}

/** What the renderer reads to draw the from/to highlight. */
interface Suggestion {
	from: Coords;
	to: Coords;
	/** `gamefile.moves.length` when this was generated. Used to expire it. */
	moveCount: number;
}
...
interface ChatMessage {
	...
	content: string;
	/**
	 * True for a turn the model needs to see but the player shouldn't.
	 *
	 * Only the session kickoff uses this. Dropping it from the history instead would
	 * leave the coach's opening reply as a leading 'assistant' turn, which the proxy
	 * discards to keep the strict alternation Anthropic requires — so the persona and
	 * the whole skill assessment would vanish on the player's very first question.
	 */
	hidden?: boolean;
}

/** A move the AI opponent settled on, ready to be turned into a MoveDraft. */
interface OpponentMove {
	from: Coords;
	to: Coords;
	/** The raw type name the model asked to promote to, if any. */
	promotion?: string;
	/** A short remark from the model, shown in the chat log. */
	comment?: string;
	/** True when the model never produced a legal move and we fell back to a random one. */
	wasRandom: boolean;
	/**
	 * Why we fell back, when we did. Shown to the player instead of a bare "no legal
	 * move": "your endpoint didn't answer in 30s" and "your model can't follow the
	 * format" both end in a random move but need completely different fixes.
	 */
	failure?: string;
	/** How many model replies it took. 0 means we went straight to the fallback. */
	attempts: number;
	/** Wall-clock time the whole turn took, so the panel can show where a slow move went. */
	elapsedMillis?: number;
	/** How many requests our proxy had to make upstream for the reply we used. */
	upstreamAttempts?: number;
}

/**
 * What the coach concluded about the player's strength, kept between games.
 *
 * The persona in `tsc.md` opens by asking three questions and then settles on an Elo
 * number. That interview is worth sitting through once, not once per game — so the
 * verdict is written here and replayed to the coach on every later session, which then
 * skips straight to playing at the right strength.
 */
interface PlayerProfile {
	/** The rating the coach settled on. Clamped to a sane range before it's stored. */
	elo: number;
	/** The coach's own words about the player: strengths, weaknesses, what they want to work on. */
	summary: string;
	/** `Date.now()` when the assessment was made. */
	assessedAt: number;
}
...
const VALID_PROTOCOLS: Protocol[] = ['openai', 'anthropic'];
 ...
 */
const PROMOTION_PREFERENCE: RawType[] = [
...
let config: AICoachConfig | undefined;
 ...
 */
function getConfig(): AICoachConfig {
	...
	const merged: AICoachConfig = { ...DEFAULT_CONFIG };
	if (saved !== undefined && saved !== null && typeof saved === 'object') {
		if (VALID_PROTOCOLS.includes(saved.protocol)) merged.protocol = saved.protocol;
		if (typeof saved.baseUrl === 'string') merged.baseUrl = saved.baseUrl;
		if (typeof saved.apiKey === 'string') merged.apiKey = saved.apiKey;
		if (typeof saved.model === 'string') merged.model = saved.model;
		if (typeof saved.moveModel === 'string') merged.moveModel = saved.moveModel;
		if (VALID_REASONING_EFFORTS.includes(saved.reasoningEffort)) merged.reasoningEffort = saved.reasoningEffort;
		if (Number.isFinite(saved.temperature)) merged.temperature = Math.min(Math.max(saved.temperature, 0), 2);
		if (Number.isInteger(saved.maxTokens)) merged.maxTokens = Math.min(Math.max(saved.maxTokens, 256), 32000);
		if (typeof saved.autoAnalyze === 'boolean') merged.autoAnalyze = saved.autoAnalyze;
		if (typeof saved.autoCritique === 'boolean') merged.autoCritique = saved.autoCritique;
	}
...
function saveConfig(changes: Partial<AICoachConfig>): AICoachConfig {
	const merged: AICoachConfig = { ...getConfig(), ...changes };
	config = merged;
 ...
 */
function getProfile(): PlayerProfile | undefined {
	if (profileLoaded) return playerProfile;
	profileLoaded = true;
	...
	if (saved === undefined || saved === null || typeof saved !== 'object') return undefined;

	const elo = Number(saved.elo);
	if (!Number.isFinite(elo) || elo < MIN_ELO || elo > MAX_ELO) return undefined;
...
function saveProfile(elo: number, summary: string): PlayerProfile {
 ...
 */
function absorbAssessment(reply: string, allowProse: boolean = true): string {
	...
	if (tagged !== null) {
		...
		if (Number.isFinite(elo) && elo >= MIN_ELO && elo <= MAX_ELO) saveProfile(elo, reply.replace(ELO_TAG_REGEX, ' '));
	...
	if (allowProse && getProfile() === undefined && chatHistory.length <= 6) {
		...
		if (prose !== null) {
			...
			if (Number.isFinite(elo) && elo >= MIN_ELO && elo <= MAX_ELO) saveProfile(elo, reply);
		}
	}
	return reply;
}
...
function serializeLimit(n: number): number | string {
 ...
 */
function nextPlayer(gf: any): Player {
	const turnOrder: Player[] = gf.gameRules.turnOrder;
	const index = turnOrder.indexOf(gf.whosTurn);
	if (index === -1) return gf.whosTurn;
 ...
 */
function namePlayerGroup(group: Record<string, any> | undefined): Record<string, any> | undefined {
	if (group === undefined || group === null) return undefined;
	const out: Record<string, any> = {};
	for (const key of Object.keys(group)) {
		out[colorName(Number(key) as Player)] = group[key];
	}
 ...
 */
function collectPieces(gf: any, includeListing: boolean, onlyColor?: string) {
	const listing: Record<string, Record<string, Coords[]>> = {};
	const counts: Record<string, Record<string, number>> = {};
	let total = 0;

	for (const [type, range] of gf.pieces.typeRanges as Map<number, { start: number, end: number }>) {
		...
		for (let idx = range.start; idx < range.end; idx++) {
			...
			if (!piece) continue; // Deleted pieces stay in the array so others keep their index
			total++;

			const colorCounts = counts[color] ??= {};
			colorCounts[name] = (colorCounts[name] ?? 0) + 1;

			if (!includeListing) continue;
 ...
 */
function collectLegalMoves(gf: any, maxPieces: number = MAX_DETAILED_PIECES, horizon?: number) {
	const mover: Player = gf.whosTurn;
	const pieces: any[] = [];
	let truncated = false;
	...
	for (const rawType of Object.values(rawTypes)) {
		...
		if (!range) continue; // This game has no piece of this type

		for (let idx = range.start; idx < range.end; idx++) {
			if (pieces.length >= maxPieces) { truncated = true; break; }

			const piece = boardutil.getPieceFromIdx(gf.pieces, idx);
			if (!piece) continue;

			const moves = legalmoves.calculate(gf, piece);
			if (!legalmoves.hasAtleast1Move(moves)) continue; // Nothing worth reporting
			...
			const jumps: Coords[] = (moves.individual ?? []).map((c: Coords) => [c[0], c[1]] as Coords);
			if (jumps.length > 0) entry.jumpTo = jumps;

			// Sliding moves: 'dx,dy' -> [minSteps, maxSteps]
			const slides: Record<string, (number | string)[]> = {};
			const captures: Coords[] = [];
			for (const dirKey of Object.keys(moves.sliding ?? {})) {
				const limits = moves.sliding[dirKey] as [number, number];
				if (limits[0] === 0 && limits[1] === 0) continue; // Fully blocked both ways
				// The advertised range may be narrowed; `limits` stays true, so the
				// capture scan below still finds captures beyond the horizon.
				const shown: [number, number] = horizon === undefined
					? limits
				...
				const dir = coordutil.getCoordsFromKey(dirKey as `${number},${number}`);
				for (const steps of limits) {
					const square = capturableSquare(gf, piece.coords, dir, steps, mover);
					if (square !== undefined) captures.push(square);
				}
			}
			if (Object.keys(slides).length > 0) entry.slideRanges = slides;
			...
			for (const square of jumps) {
				...
				if (typeutil.getColorFromType(occupant) === mover) continue;
				captures.push(square);
			}
			if (captures.length > 0) entry.canCapture = captures;
...
function capturableSquare(gf: any, from: Coords, dir: Coords, steps: number, mover: Player): Coords | undefined {
	if (!Number.isFinite(steps) || steps === 0) return undefined;
	const square: Coords = [from[0] + steps * dir[0], from[1] + steps * dir[1]];
	const occupant = boardutil.getTypeFromCoords(gf.pieces, square);
	if (occupant === undefined) return undefined;
	if (typeutil.getColorFromType(occupant) === mover) return undefined;
	return square;
}
 ...
 */
function renderBoardMap(gf: any): { rows: string[], legend: string, walled: boolean } | undefined {
	...
	if (left === Infinity) return undefined; // Nothing but scenery
	...
	if (right - left + 1 > MAX_GRID_SPAN || top - bottom + 1 > MAX_GRID_SPAN) return undefined;

	// Coordinates can be negative or two digits, so the column width follows the labels.
	const labels: string[] = [];
	for (let x = left; x <= right; x++) labels.push(String(x));
	let width = 2;
	for (const label of labels) width = Math.max(width, label.length);
	for (let y = bottom; y <= top; y++) width = Math.max(width, String(y).length);
	const cell = (text: string): string => text.padStart(width, ' ');

	const rows: string[] = [`${cell('')}|${labels.map(cell).join(' ')}`];
	let walledWindow = false;
	for (let y = top; y >= bottom; y--) {
		const cells: string[] = [];
		for (let x = left; x <= right; x++) {
			const type = boardutil.getTypeFromCoords(gf.pieces, [x, y]);
			if (type === undefined) { cells.push(cell('.')); continue; }
			if (isWallType(type)) { walledWindow = true; cells.push(cell('##')); continue; }
...
function enumerateDestinations(from: Coords, legal: any, horizon: number): { squares: Coords[], clamped: boolean } {
	const squares: Coords[] = [];
	let clamped = false;

	for (const coords of legal?.individual ?? []) {
		if (Array.isArray(coords) && coords.length >= 2) squares.push([Number(coords[0]), Number(coords[1])]);
	}
	...
	if (sliding !== undefined && sliding !== null) {
		for (const key of Object.keys(sliding)) {
			...
			const dir = coordutil.getCoordsFromKey(key as `${number},${number}`);
			let min = Number(limits[0]);
			let max = Number(limits[1]);
			if (min < -horizon) { min = -horizon; clamped = true; }
			if (max > horizon) { max = horizon; clamped = true; }
...
/** One fully enumerated legal-move list, ready to be handed to the model verbatim. */
interface EnumeratedMoves {
	/** Every legal move, as 'fromX,fromY>toX,toY'. */
	moves: string[];
	/** The subset of {@link EnumeratedMoves.moves} that captures something. */
	captures: string[];
	/** The subset that promotes a pawn. */
	promotions: string[];
	/** True when no sliding range had to be clamped, i.e. the list really is every legal move. */
	exhaustive: boolean;
}
 ...
 */
function enumerateLegalMoves(gf: any, horizon: number): EnumeratedMoves | undefined {
	const mover: Player = gf.whosTurn;
	const moves: string[] = [];
	...
	for (const rawType of Object.values(rawTypes)) {
		...
		for (let idx = range.start; idx < range.end; idx++) {
			...
			if (!legalmoves.hasAtleast1Move(legal)) continue;

			const { squares, clamped } = enumerateDestinations(piece.coords, legal, horizon);
			if (clamped) exhaustive = false;
			...
			for (const to of squares) {
				...
				const coords = coordutil.copyCoords(to) as CoordsSpecial;
				if (!legalmoves.checkIfMoveLegal(gf, legal, piece.coords, coords, mover)) continue;

				seen.add(key);
				moves.push(key);
				if (moves.length > MAX_ENUMERATED_MOVES) return undefined;
				...
				if (takesSomething) captures.push(key);
				if (coords.promoteTrigger) promotions.push(key);
			}
		}
	}

	if (moves.length === 0) return undefined; // Stalemate/checkmate; let the range path report it
 ...
 */
function describePosition(gf: any, options: { compact?: boolean } = {}): Record<string, any> {
	const compact = options.compact === true;
	const notes: string[] = [];

	const totalPieces = boardutil.getPieceCountOfGame(gf.pieces);
	const includeListing = totalPieces <= MAX_PIECES_LISTED;
	if (!includeListing) notes.push(`Board has ${totalPieces} pieces — too many to list individually, only material counts are given.`);
	...
	let legalMoveData: any;
	if (exact !== undefined) {
		legalMoveData = exact.moves;
		if (!exact.exhaustive) {
			notes.push(
				'legalMoves is complete except that sliding moves are only listed out to a few steps. '
				+ 'Sliding further is legal but almost never good — the piece stops defending anything.',
			);
		}
	} else {
		const ranges = collectLegalMoves(
			gf,
			compact ? COMPACT_DETAILED_PIECES : MAX_DETAILED_PIECES,
			compact ? COMPACT_SLIDE_HORIZON : undefined,
		);
		legalMoveData = ranges.pieces;
		if (ranges.truncated) notes.push(`Legal moves listed for only the first ${ranges.maxPieces} pieces.`);
		if (compact) {
			notes.push(
				`slideRanges are capped at ${COMPACT_SLIDE_HORIZON} steps in each direction. Sliding further is legal `
	...
	const turnOrder: Player[] = gf.gameRules.turnOrder;
	const halfMovesPlayed: number = gf.moves.length;
	const fullMove: number = (gf.startSnapshot?.fullMove ?? 1) + Math.floor(halfMovesPlayed / turnOrder.length);

	// Full move history, most compact form: '8,7>8,8=Q'
	const allMoves: string[] = gf.moves.map((m: any) => m.compact);
	const history = allMoves.slice(compact ? -COMPACT_HISTORY_MOVES : -MAX_HISTORY_MOVES);
	if (history.length < allMoves.length) notes.push(`Only the last ${history.length} of ${allMoves.length} moves are shown.`);
	...
	if (exact !== undefined) {
		if (exact.captures.length > 0) desc['capturingMoves'] = exact.captures;
		if (exact.promotions.length > 0) desc['promotingMoves'] = exact.promotions;
	}
	...
	if (listing !== undefined) {
		desc['pieces'] = listing;
		if (compact) desc['piecesNote'] = 'Only the opposing side is listed here; your own pieces appear in legalMoves with their coordinates.';
	}

	// Rules that change how the position should be judged.
	const rules: Record<string, any> = {};
	const winConditions = namePlayerGroup(gf.gameRules.winConditions);
	if (winConditions !== undefined) rules['winConditions'] = winConditions;
	const promotionRanks = namePlayerGroup(gf.gameRules.promotionRanks);
	if (promotionRanks !== undefined) rules['promotionRanks'] = promotionRanks;
	...
	if (promotionsAllowed !== undefined) {
		...
		for (const color of Object.keys(promotionsAllowed)) {
			...
			if (Array.isArray(list)) readable[color] = list.map((t: number) => typeutil.getRawTypeStr(t as any));
		}
	...
	if (gf.gameRules.slideLimit !== undefined) rules['slideLimit'] = gf.gameRules.slideLimit;
	...
	if (notes.length > 0) desc['payloadNotes'] = notes;
	return desc;
}
...
let personaLoaded: boolean = false;
 ...
 */
async function loadPersona(): Promise<boolean> {
	if (personaLoaded) return persona !== '';
	personaLoaded = true;
...
function getPersonaError(): string | undefined {
	return personaError;
}
...
function buildUserPrompt(desc: Record<string, any>, language: string, retryHint?: string): string {
	const parts: string[] = [];
	parts.push(`Here is the current position. Analyze it for ${desc['toMove']}, who is to move.`);
	parts.push('```json\n' + JSON.stringify(desc) + '\n```');
	if (retryHint !== undefined) parts.push(retryHint);
 ...
 */
function buildOpponentSystemPrompt(desc: Record<string, any>): string {
...
function buildOpponentPrompt(desc: Record<string, any>, language: string, rejected: string[]): string {
	const parts: string[] = [];
	parts.push(`You are playing as ${desc['toMove']} and it is your move. Here is the position.`);
	parts.push('```json\n' + JSON.stringify(desc) + '\n```');
	if (rejected.length > 0) {
		parts.push(
			`Every candidate in your previous ${rejected.length === 1 ? 'reply' : 'replies'} was rejected as illegal:\n`
...
function buildProfileBlock(): string {
	...
	if (profile.summary !== '') parts.push(`- What you noted about them: ${profile.summary}`);
	return parts.join('\n');
}
 ...
 * @returns The block, or '' when the player has never been assessed
 */
function buildOpponentProfileBlock(): string {
	const profile = getProfile();
	if (profile === undefined) return '';
 ...
 */
function buildKickoffPrompt(language: string): string {
	const profile = getProfile();
...
function buildChatPrompt(question: string, desc: Record<string, any> | undefined, language: string): string {
	if (desc === undefined) return question;
	const parts: string[] = [];
 ...
 * of the whole game.
 */
function buildCritiquePrompt(desc: Record<string, any> | undefined, language: string, final: boolean): string {
	const parts: string[] = [];
	if (desc !== undefined) {
		parts.push('Current position:');
		parts.push('```json\n' + JSON.stringify(desc) + '\n```');
	...
	if (final) {
		parts.push(
			`The game is over. Give me a short review of how I played: the one or two habits that `
			+ `cost me the most, and the single thing to work on next game. Three or four sentences.`,
		);
	} else {
		parts.push(
			`I just moved. Without being asked, tell me what you think of my play so far: `
...
class ProxyError extends Error {
	...
	constructor(message: string, timedOut: boolean = false) {
		super(message);
 ...
 */
async function callProxyMessages(
	system: string,
	messages: ChatMessage[],
	overrides: { model?: string, reasoningEffort?: ReasoningEffort, timeoutMillis?: number } = {},
): Promise<string> {
	const c = getConfig();
	if (!isConfigured()) throw new ProxyError(translate('needs_config', 'Fill in the API URL, key and model first.'));
	...
	try {
		...
		if (error?.name === 'AbortError') {
			throw new ProxyError(
				`${translate('timed_out', 'The AI endpoint did not answer in time')} (${Math.round((abortAfter ?? 0) / 1000)}s).`,
				true,
			);
		}
		throw new ProxyError(error?.message ?? 'Request failed.');
	} finally {
		if (abortID !== undefined) clearTimeout(abortID);
	}
	...
	if (!response.ok) throw new ProxyError(json?.message ?? `HTTP ${response.status}`, json?.timedOut === true);
	if (typeof json?.text !== 'string') throw new ProxyError('Server returned no text.');
 ...
 */
let lastUpstreamAttempts: number = 1;
...
async function testConnection(): Promise<string> {
	const text = await callProxy(
		'You are a connection test. Reply with exactly the word OK and nothing else.',
		'Reply with OK.',
	);
...
function toCoords(value: any): Coords | undefined {
...
function toProposedMove(value: any): ProposedMove | undefined {
	...
	const move: ProposedMove = { from, to };
	if (typeof value.note === 'string' && value.note !== '') move.note = value.note;
	return move;
}
...
function parseAdvice(raw: string): Advice {
	...
	if (json === undefined || json === null || typeof json !== 'object') return advice;

	if (typeof json.assessment === 'string') advice.assessment = json.assessment;
	if (typeof json.reasoning === 'string') advice.reasoning = json.reasoning;
	if (Array.isArray(json.threats)) {
		advice.threats = json.threats.filter((t: any) => typeof t === 'string' && t !== '');
...
/** One candidate move out of the opponent's reply, before we've validated it. */
 ...
 */
function promotionName(raw: string): string {
	try {
		const type = icnconverter.getTypeFromAbbr(raw);
		if (Number.isInteger(type)) return typeutil.getRawTypeStr(typeutil.getRawType(type));
	} catch {
...
function parseMoveString(value: string): CandidateMove | undefined {
	const match = MOVE_STRING_REGEX.exec(value);
	if (match === null) return undefined;
	const nums = [match[1], match[2], match[3], match[4]].map((part) => Number(part));
	if (!nums.every((n) => Number.isInteger(n))) return undefined;
	const candidate: CandidateMove = { from: [nums[0]!, nums[1]!], to: [nums[2]!, nums[3]!] };
	const promotion = match[5];
	if (promotion !== undefined && promotion !== '') candidate.promotion = promotionName(promotion);
	return candidate;
}
 ...
 */
function parseChosenMoves(raw: string): { candidates: CandidateMove[], comment?: string } {
	const json = extractJSON(raw);
	if (json === undefined || json === null || typeof json !== 'object') return { candidates: [] };

	const result: { candidates: CandidateMove[], comment?: string } = { candidates: [] };

	const topComment = json.comment ?? json.note ?? json.message;
	if (typeof topComment === 'string' && topComment !== '') result.comment = topComment.slice(0, 400);

	/** Pushes an entry if it parses into a move, skipping duplicates. */
	function consider(entry: any): void {
		let move: CandidateMove | undefined;
	...
	if (Array.isArray(listed)) for (const entry of listed) consider(entry);
 ...
 */
function validateMove(gf: any, from: Coords, to: Coords): string | undefined {
	if (!coordutil.areCoordsIntegers(from) || !coordutil.areCoordsIntegers(to)) {
		return 'coordinates must be integers';
	}
	if (coordutil.areCoordsEqual_noValidate(from, to)) return 'the from and to squares are the same';

	const piece = boardutil.getPieceFromCoords(gf.pieces, from);
	if (piece === undefined) return `there is no piece on [${from[0]},${from[1]}]`;

	const color: Player = typeutil.getColorFromType(piece.type);
	if (color !== gf.whosTurn) return `the piece on [${from[0]},${from[1]}] is not yours to move`;
	...
	const legal: boolean = legalmoves.checkIfMoveLegal(gf, legalMoves, piece.coords, endCoords, color);
	if (!legal) return `${typeutil.getRawTypeStr(typeutil.getRawType(piece.type))} on [${from[0]},${from[1]}] cannot reach [${to[0]},${to[1]}]`;
 ...
 */
function choosePromotion(gf: any, color: Player, requested?: string): number | undefined {
	const allowed: RawType[] | undefined = gf.gameRules?.promotionsAllowed?.[color];
	if (allowed === undefined || allowed.length === 0) return undefined;

	if (requested !== undefined) {
		const wanted = requested.trim().toLowerCase();
		for (const raw of allowed) {
			if (typeutil.getRawTypeStr(raw).toLowerCase() === wanted) return typeutil.buildType(raw, color);
		}
	}

	// The model asked for nothing, or for something this variant doesn't offer.
	for (const preferred of PROMOTION_PREFERENCE) {
		if (allowed.includes(preferred)) return typeutil.buildType(preferred, color);
	}
 ...
 */
function buildMoveDraft(gf: any, from: Coords, to: Coords, promotion?: string): { draft?: MoveDraft, reason?: string } {
	if (!coordutil.areCoordsIntegers(from) || !coordutil.areCoordsIntegers(to)) {
		return { reason: 'coordinates must be integers' };
	}
	if (coordutil.areCoordsEqual_noValidate(from, to)) return { reason: 'the from and to squares are the same' };

	const piece = boardutil.getPieceFromCoords(gf.pieces, from);
	if (piece === undefined) return { reason: `there is no piece on [${from[0]},${from[1]}]` };

	const color: Player = typeutil.getColorFromType(piece.type);
	if (color !== gf.whosTurn) return { reason: `the piece on [${from[0]},${from[1]}] is not yours to move` };
 ...
 */
function collectCandidateDestinations(from: Coords, legalMoves: any): Coords[] {
	const out: Coords[] = [];

	if (Array.isArray(legalMoves?.individual)) {
		for (const coords of legalMoves.individual) {
			if (Array.isArray(coords) && coords.length >= 2) out.push([Number(coords[0]), Number(coords[1])]);
		}
	}
	...
	if (sliding !== undefined && sliding !== null) {
		for (const key of Object.keys(sliding)) {
			const direction = coordutil.getCoordsFromKey(key as any);
 ...
 */
function pickRandomLegalMove(gf: any): MoveDraft | undefined {
	const mover: Player = gf.whosTurn;
	...
	let wanderingFallback: MoveDraft | undefined;
	for (const piece of shuffled(candidates)) {
		const legalMoves = legalmoves.calculate(gf, piece);
		if (!legalmoves.hasAtleast1Move(legalMoves)) continue;
		...
		for (const to of shuffled(collectCandidateDestinations(piece.coords, legalMoves))) {
			...
			if (!isWanderingMove(gf, piece.coords, to)) return draft;
...
let latestAdvice: Advice | undefined;
 ...
 */
function getSuggestion(gf?: any): Suggestion | undefined {
...
function getLatestAdvice(): Advice | undefined {
	return latestAdvice;
}

function isRequestInFlight(): boolean {
...
function pushChatMessage(role: 'user' | 'assistant', content: string, hidden: boolean = false): void {
 ...
 */
async function startCoachSession(): Promise<string | undefined> {
	if (coachSessionStarted || coachSessionStarting) return undefined;
	if (!isConfigured()) return undefined;
	if (chatHistory.length > 0) { coachSessionStarted = true; return undefined; } // Mid-conversation already
	if (!await loadPersona()) return undefined;
	...
	try {
		...
		const greeting = absorbAssessment((await callProxyMessages(
 ...
 */
async function requestAdvice(gf: any): Promise<Advice> {
	if (requestInFlight) throw new Error(translate('busy', 'An analysis is already running.'));
	...
	try {
		const moveCountAtRequest: number = gf.moves.length;
		...
		if (advice.bestMove !== undefined) {
			...
			if (reason !== undefined) {
				...
				if (retried.bestMove !== undefined && validateMove(gf, retried.bestMove.from, retried.bestMove.to) === undefined) {
					...
					if (retried.assessment !== '') advice = { ...retried, bestMove: advice.bestMove };
 ...
 */
function occupiedBounds(gf: any): { left: number, right: number, bottom: number, top: number } {
	let left = Infinity, right = -Infinity, bottom = Infinity, top = -Infinity;
	for (const range of (gf.pieces.typeRanges as Map<number, { start: number, end: number }>).values()) {
		for (let idx = range.start; idx < range.end; idx++) {
			const piece = boardutil.getPieceFromIdx(gf.pieces, idx);
			if (!piece) continue;
			const [x, y] = piece.coords;
 ...
 */
function isWanderingMove(gf: any, from: Coords, to: Coords): boolean {
	if (boardutil.getTypeFromCoords(gf.pieces, to) !== undefined) return false; // A capture
	const travel = Math.max(Math.abs(to[0] - from[0]), Math.abs(to[1] - from[1]));
	if (travel > COMPACT_SLIDE_HORIZON) return true;
 ...
 */
async function requestOpponentMove(gf: any): Promise<{ draft: MoveDraft, meta: OpponentMove } | undefined> {
	...
	for (let attempt = 1; attempt <= MAX_MOVE_ATTEMPTS; attempt++) {
		...
		for (const pass of [0, 1]) {
			for (const candidate of pass === 0 ? candidates : deferred) {
				...
				if (draft === undefined) {
					if (pass === 0) rejected.push(`[${candidate.from[0]},${candidate.from[1]}] to [${candidate.to[0]},${candidate.to[1]}] — ${reason}`);
					continue;
				}
				const meta: OpponentMove = {
					from: candidate.from,
					to: candidate.to,
					wasRandom: false,
					attempts: attempt,
				...
				if (candidate.promotion !== undefined) meta.promotion = candidate.promotion;
	...
	if (lastError !== undefined) console.warn('AI opponent request failed, playing a random move instead.');
	...
	if (failure !== undefined) meta.failure = failure;
 ...
 */
function describeMoveFailure(lastError: unknown, rejected: string[]): string | undefined {
 ...
 */
async function askQuestion(gf: any, question: string): Promise<string> {
	const trimmed = question.trim();
	if (trimmed === '') throw new Error(translate('chat_empty', 'Type a question first.'));
 ...
 */
async function requestCritique(gf: any, final: boolean = false): Promise<string> {
	const language = document.documentElement.lang || 'en-US';
	const desc = gf === undefined ? undefined : describePosition(gf);
