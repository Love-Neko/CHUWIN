...
function onmessage(req, ws, rawMessage) {
    ...
    try {
        ...
        if (!rateLimitAndLogMessage(req, ws, rawMessage))
            return; // The socket will have already been closed.
    ...
    if (printIncomingAndOutgoingMessages && !isEcho)
 ...
 */
function rateLimitAndLogMessage(req, ws, rawMessage) {
    if (!rateLimitWebSocket(req, ws))
        return false; // They are being rate limited, the socket will have already been closed.
 ...
 */
function isValidWebsocketInMessage(parsedIncomingMessage) {
    return (typeof parsedIncomingMessage === 'object' &&
