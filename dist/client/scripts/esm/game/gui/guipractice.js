...
const element_progress = document.querySelector('.checkmate-progress');
const element_progressBar = document.querySelector('.checkmate-progress-bar');
const element_checkmateList = document.querySelector('.checkmate-list');
const element_checkmates = document.getElementById('checkmates');
const element_checkmateBadgeBronze = document.getElementById('checkmate-badge-bronze');
const element_checkmateBadgeBronzeImage = document.querySelector('#checkmate-badge-bronze img');
...
const element_checkmateBadgeSilverImage = document.querySelector('#checkmate-badge-silver img');
...
const element_checkmateBadgeGoldImage = document.querySelector('#checkmate-badge-gold img');
...
let checkmateSelectedID = validcheckmates.validCheckmates.easy[0]; // id of selected checkmate
...
document.addEventListener('theme-change', () => {
    ...
    if (isOpen)
        addPieceIcons(); // Regenerate the icons so they can update their color, if the new theme has different color arguments
});
/**
...
function createPracticeHTML() {
    for (const [difficulty, checkmates] of Object.entries(validcheckmates.validCheckmates)) {
        checkmates.forEach((checkmateID) => {
            ...
            for (const entry of piecelist) {
                const amount = parseInt(entry.match(/[0-9]+/)[0]); // number of pieces to be placed
                const shortPiece = entry.match(/[a-zA-Z]+/)[0]; // piecetype to be placed
                ...
                for (let j = 0; j < amount; j++) {
                    ...
                    if (typeutil.getColorFromType(longPiece) === players.WHITE)
                        ...
                        piecelistB.appendChild(containerDiv);
                }
            }
...
function initListeners() {
    ...
    for (const element of element_checkmates.children) {
        element.addEventListener('mouseup', callback_mouseUp);
...
function closeListeners() {
    ...
    for (const element of element_checkmates.children) {
        element.removeEventListener('mouseup', callback_mouseUp);
...
function callback_mouseDown(event) {
...
function callback_mouseUp(event) {
    ...
    if (!event.currentTarget.id)
        return; // mouse not on checkmate target
...
function callback_mouseMove(event) {
    SCROLL.mouseMovedAfterClick = true;
    if (!SCROLL.mouseIsDown)
        return;
 ...
 */
function updateBadges(numCompleted, numTotal) {
    ...
    badgeConfigs.forEach(config => {
        if (!config.element || !config.image)
            return;
...
function callback_keyPress(event) {
    if (event.key === 'Enter')
        ...
        moveDownSelection(event);
    else if (event.key === 'ArrowUp')
        moveUpSelection(event);
}
function moveDownSelection(event) {
    event.preventDefault();
    if (indexSelected >= element_checkmates.children.length - 1)
        return;
    clearScrollMomentumInterval();
    indexSelected++;
