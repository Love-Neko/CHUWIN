...
function setTerminationMetadata(gamefile) {
    ...
    const conditionInPlainEnglish = winconutil.getTerminationInEnglish(gamefile, victorAndCondition.condition);
 ...
 */
function eraseTerminationMetadata(gamefile) {
...
function isOpponentUsingWinCondition(gamefile, friendlyColor, winCondition) {
    if (!winconutil.isWinConditionValid(winCondition))
        throw new Error(`Cannot test if opponent of color "${friendlyColor}" is using invalid win condition "${winCondition}"!`);
...
function doGameOverChecks(gamefile) {
    ...
    if (isGameOver(gamefile) && winconutil.isGameConclusionDecisive(gamefile.gameConclusion))
 ...
 */
function getStartingAreaBox(gamefile) {
    if (gamefile.startSnapshot?.box)
        return gamefile.startSnapshot.box;
 ...
 */
function areColinearSlidesPresentInGame(pieceMovesets, slides) {
    /**
     ...
     */
    if (slides.some((vector) => math.GCD(vector[0], vector[1]) !== 1))
...
function getPlayerCount(gamefile) {
    if (gamefile.startSnapshot)
        return gamefile.startSnapshot.playerCount;
