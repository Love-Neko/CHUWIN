...
import input2, { Mouse } from "../game/input.js";
 ...
 */
function getPointerPosition_Offscreen(pointerId) {
    if (pointerId === 'mouse') {
...
function getMouseWorld() {
    if (!perspective.getEnabled()) {
        ...
        return getCrossHairWorld(); // Mouse is locked, we must be in perspective mode. Calculate the mouse world according to the crosshair location instead.
}
/**
 * Returns the world space coordinates of the mouse pointer,
 * or the crosshair if in perspective mode.
 */
function getPointerWorld(pointerId) {
    if (!perspective.getEnabled()) {
...
function convertMousePositionToWorldSpace(mouse, element) {
    ...
    const mouseWorldSpace = perspective.getIsViewingBlackPerspective() ? [
...
function getTilePointerOver_Integer(pointerId) {
    // const pointerCoords = listener_overlay.getPointerPos(pointerId)!;
    const pointerCoords = getPointerPosition_Offscreen(pointerId);
    if (!pointerCoords)
        return undefined;
...
function isMouseDown(button) {
    if (perspective.isMouseLocked())
        return listener_document.isMouseDown(button);
    else
        return listener_overlay.isMouseDown(button);
}
/**
 * Wrapper for reading the correct listener for whether the mouse button is held,
 * depending on whether we're in perspective mode or not.
 */
function isMouseHeld(button) {
    if (perspective.isMouseLocked())
        return listener_document.isMouseHeld(button);
    else
        return listener_overlay.isMouseHeld(button);
}
/**
...
function isMouseClicked(button) {
    if (perspective.isMouseLocked())
        return listener_document.isMouseClicked(button);
    else
        return listener_overlay.isMouseClicked(button);
}
/**
 * Wrapper for reading the correct listener for whether the mouse button was double-click simulated,
 * depending on whether we're in perspective mode or not.
 */
function isMouseDoubleClickDragged(button) {
    if (perspective.isMouseLocked())
        return listener_document.isMouseDoubleClickDragged(button);
    else
        return listener_overlay.isMouseDoubleClickDragged(button);
}
/**
...
function getWheelDelta() {
    if (perspective.isMouseLocked())
        return listener_document.getWheelDelta();
    else
        return listener_overlay.getWheelDelta();
}
export default {
    getMouseWorld,
    getPointerWorld,
