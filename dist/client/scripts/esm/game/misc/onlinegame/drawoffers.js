...
function isOfferingDrawLegal() {
    ...
    if (!onlinegame.areInOnlineGame())
        return false; // Can't offer draws in local games
    if (!moveutil.isGameResignable(gamefile))
        return false; // Not atleast 2+ moves
    if (onlinegame.hasServerConcludedGame())
        return false; // Can't offer draws after the game has ended
    if (isTooSoonToOfferDraw())
        return false; // It's been too soon since our last offer
...
function isTooSoonToOfferDraw() {
    const gamefile = gameslot.getGamefile();
    if (plyOfLastOfferedDraw === undefined)
        return false; // We have made zero offers so far this game
    const movesSinceLastOffer = gamefile.moves.length - plyOfLastOfferedDraw;
    if (movesSinceLastOffer < movesBetweenDrawOffers)
        return true;
    return false;
}
/**
...
function callback_declineDraw() {
    if (!isAcceptingDraw)
        return; // No open draw offer from our opponent
    closeDraw();
    // Notify the server
...
function set(drawOffer) {
    ...
    if (!drawOffer.unconfirmed)
        return; // No open draw offer
    // Open draw offer!!
