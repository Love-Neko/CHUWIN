...
function playSound(soundName, { volume = 1, delay = 0, offset = 0, fadeInDuration = 0, reverbVolume = 0, reverbDuration = 0 } = {}) {
    // A reverb volume of 3.5 and a duration of 1.5 seconds most-closely matches my audio file!
    if (!htmlscript.hasUserGesturedAtleastOnce())
        return; // Skip playing this sound 
    if (!audioContext)
        throw Error(`Can't play sound ${soundName} when audioContext isn't initialized yet. (Still loading)`);
    ...
    if (duration < 0)
        return; // Offset is greater than the sound length, the sound is already over.
    ...
    const soundObject = {
        ...
        fadeOut: (durationMillis) => {
            ...
            if (soundObject.sourceReverb)
                fadeOut(soundObject.sourceReverb, durationMillis);
        }
    };
    ...
    if (!reverbVolume)
        return fadeInAndReturn(); // No reverb effect if volume is falsey or zero :)
    if (!reverbDuration)
        throw Error("Need to specify a reverb duration.");
    ...
    function fadeInAndReturn() {
        if (!fadeInDuration)
            return soundObject; // No fade-in effect
        fadeIn(soundObject.source, volume, fadeInDuration);
        if (soundObject.sourceReverb)
            fadeIn(soundObject.sourceReverb, reverbVolume, fadeInDuration);
        return soundObject;
    }
}
function getSoundStamp(soundName) {
    ...
    if (stamp)
        ...
        throw new Error(`Cannot return sound stamp for strange new sound ${soundName}!`);
}
...
function createBufferSource(volume, playbackRate = 1, reverbDurationSecs) {
    ...
    if (!audioDecodedBuffer)
        throw new Error("audioDecodedBuffer should never be undefined! This usually happens when soundspritesheet.mp3 starts loading but the document finishes loading in the middle of the audio loading.");
...
function fadeIn(source, targetVolume, fadeDuration) {
    if (!source?.gainNode)
        throw new Error("Source or gain node not provided");
    const currentTime = audioContext.currentTime;
...
function fadeOut(source, durationMillis) {
    if (!source?.gainNode)
        throw new Error("Source or gain node not provided"); // Hopefully This should be caught by TS
...
function calculateReverbVolDurFromDistance(distanceMoved) {
    ...
    if (x <= 0)
        return { reverbVolume: undefined, reverbDuration: undefined };
    else if (x >= 1)
        return { reverbVolume: maxReverbVol, reverbDuration };
