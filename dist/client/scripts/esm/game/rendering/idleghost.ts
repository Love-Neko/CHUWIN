...
function show(millis: number): void {
	const now = Date.now();
	shownAt = now;
 ...
 */
function ensureAnchor(): number | undefined {
	if (anchorYaw === undefined && perspective.getEnabled()) anchorYaw = perspective.getRotZ();
	return anchorYaw;
}
...
function ensureTexture(): void {
	...
	image.onload = () => {
		try {
			...
			if (image.naturalHeight > 0) aspect = image.naturalWidth / image.naturalHeight;
			textureState = 'ready';
			frametracker.onVisualChange();
		} catch (error) {
			textureState = 'failed';
...
function getAlpha(now: number): number {
 ...
 */
function writeVertexData(data: Float32Array, alpha: number, yaw: number): void {
...
function render(): void {
	if (!perspective.getEnabled()) return; // There is no "behind you" in the top-down view
	if (textureState !== 'ready' || glTexture === undefined) return;
