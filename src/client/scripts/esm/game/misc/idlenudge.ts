...
let nudgedAtMoveCount: number = -1;
...
function isWaitingOnPlayer(gamefile: any): boolean {
	if (gamefileutility.isGameOver(gamefile)) return false;
	if (onlinegame.areInOnlineGame()) return onlinegame.isItOurTurn();
...
function update(): void {
	...
	if (nudgedAtMoveCount === moveCount) return; // Already stepped in on this turn
	if (now - waitingSince < IDLE_MILLIS) return;
