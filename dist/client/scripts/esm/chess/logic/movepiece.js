 ...
 */
function generateMove(gamefile, moveDraft) {
    const piece = boardutil.getPieceFromCoords(gamefile.pieces, moveDraft.startCoords);
    if (!piece)
        throw Error(`Cannot make move because no piece exists at coords ${JSON.stringify(moveDraft.startCoords)}.`);
    ...
    if (rawType in gamefile.specialMoves)
        specialMoveMade = gamefile.specialMoves[rawType](gamefile, piece, move);
    if (!specialMoveMade)
        calcMovesChanges(gamefile, piece, move); // Move piece regularly (no special tag)
...
function generateNullMove(gamefile) {
    const nullMove = {
        isNull: true,
        generateIndex: gamefile.state.local.moveIndex + 1,
        state: { local: [], global: [] },
 ...
 */
function calcMovesChanges(gamefile, piece, move) {
    const capturedPiece = boardutil.getPieceFromCoords(gamefile.pieces, move.endCoords);
    if (capturedPiece)
        boardchanges.queueCapture(move.changes, true, piece, move.endCoords, capturedPiece);
    else
 ...
 */
function queueSpecialRightDeletionStateChanges(gamefile, move) {
 ...
 */
function queueIncrementMoveRuleStateChange(gamefile, move) {
    if (!gamefile.gameRules.moveRule)
        return; // Not using the move-rule
...
function makeMove(gamefile, move) {
    ...
    if (gamefile.state.local.inCheck)
...
function applyMove(gamefile, move, forward = true, { global = false } = {}) {
    ...
    if (indexToApply !== move.generateIndex)
        throw new Error(`Move was expected at index ${move.generateIndex} but applied at ${indexToApply} (forward: ${forward}).`);
    state.applyMove(gamefile.state, move.state, forward, { globalChange: global }); // Apply the State of the move
    if (move.isNull)
        return; // Null moves don't have changes to make
...
function makeAllMovesInGame(gamefile, moves) {
    ...
    moves.forEach((shortmove, i) => {
        ...
        if (!move)
            throw Error(`Cannot make all moves in game! There was one invalid move: ${shortmove}. Index: ${i}`);
...
function calculateMoveFromShortmove(gamefile, shortmove) {
    if (!moveutil.areWeViewingLatestMove(gamefile))
        throw Error("Cannot calculate Move object from shortmove when we're not viewing the most recently played move.");
    ...
    const legalSpecialMoves = legalmoves.calculate(gamefile, piece, { onlyCalcSpecials: true }).individual;
    for (let i = 0; i < legalSpecialMoves.length; i++) {
        const thisCoord = legalSpecialMoves[i];
        if (!coordutil.areCoordsEqual(thisCoord, moveDraft.endCoords))
            continue;
 ...
 */
function simulateMoveWrapper(gamefile, moveDraft, callback) {
 ...
 */
function getSimulatedConclusion(gamefile, moveDraft) {
...
function ensureMovesNotNull(moves) {
    return moves.map((move) => {
        if (!move.isNull)
            return move;
        else
            throw Error("Should not be null moves in game!");
    });
}
export default {
    generateMove,
