...
function update() {
    ...
    for (let i = hoveredArrowsLegalMoves.length - 1; i >= 0; i--) { // Iterate backwards because we are removing elements as we go
        ...
        if (!hoveredArrows.some(arrow => arrow.piece.coords === thisHoveredArrow.piece.coords))
            hoveredArrowsLegalMoves.splice(i, 1); // No longer being hovered over
    }
 ...
 */
function onPieceIndicatorHover(piece) {
    // Check if their legal moves and mesh have already been stored
    // TODO: Make sure this is still often called
    if (hoveredArrowsLegalMoves.some(hoveredArrow => hoveredArrow.piece.coords === piece.coords))
...
function renderEachHoveredPieceLegalMoves() {
    if (hoveredArrowsLegalMoves.length === 0)
        return; // No legal moves to render
    ...
    hoveredArrowsLegalMoves.forEach(hoveredArrow => {
        ...
        if (selection.isAPieceSelected()) {
            ...
            if (coordutil.areCoordsEqual_noValidate(hoveredArrow.piece.coords, pieceSelectedCoords))
                return; // Skip (already rendering its legal moves, because it's selected)
        }
...
function regenModelsOfHoveredPieces() {
    if (hoveredArrowsLegalMoves.length === 0)
        return; // No arrows being hovered over
