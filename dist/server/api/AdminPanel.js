...
function parseArgumentsFromCommand(command) {
    // Parse command
...
function deleteCommand(command, commandAndArgs, req, res) {
    ...
    if (!result.success)
        return sendAndLogResponse(res, 500, `Failed to delete user ${username} for reason: ${result.reason}`);
    sendAndLogResponse(res, 200, "Successfully deleted user " + username + ".");
}
function usernameCommand(command, commandAndArgs, req, res) {
    if (commandAndArgs[1] === "get") {
        ...
        if (username === undefined)
            ...
            sendAndLogResponse(res, 200, username);
    }
...
function logoutUser(command, commandAndArgs, req, res) {
...
function verify(command, commandAndArgs, req, res) {
    if (commandAndArgs.length < 2) {
        res.status(422).send("Invalid number of arguments, expected 1, got " + (commandAndArgs.length - 1) + ".");
        return;
    }
    ...
    if (result.success)
        ...
        sendAndLogResponse(res, 500, result.reason); // Failure message
}
function getUserInfo(command, commandAndArgs, req, res) {
    if (commandAndArgs.length < 2) {
        res.status(422).send("Invalid number of arguments, expected 1, got " + (commandAndArgs.length - 1) + ".");
        return;
    }
...
function logCommand(command, req) {
    if (req.memberInfo.signedIn) {
        ...
        throw new Error('Admin SHOULD have been logged in by this point. DANGEROUS');
}
function sendAndLogResponse(res, code, message) {
