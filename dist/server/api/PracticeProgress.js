 ...
 */
function setPracticeProgressCookie(req, res, next) {
    if (!req.cookies) {
    ...
    if (req.headers['is-fetch-request'] === 'true' || !req.accepts('html'))
        return next(); // Not an HTML request (but a fetch), don't set the cookie
    ...
    if (memberInfoCookieStringified === undefined)
        return next(); // No cookie is present, not logged in
    let memberInfoCookie;
    try {
        memberInfoCookie = JSON.parse(memberInfoCookieStringified);
    }
    catch (error) {
        logEvents(`memberInfo cookie was not JSON parse-able when attempting to set checkmates_beaten cookie. Maybe it was tampered? The cookie: "${jsutil.ensureJSONString(memberInfoCookieStringified)}" The error: ${error.stack}`, 'errLog.txt', { print: true });
 ...
 */
function createPracticeProgressCookie(res, checkmates_beaten) {
 ...
 */
function deletePracticeProgressCookie(res) {
 ...
 */
function getCheckmatesBeaten(userId) {
 ...
 */
function checkmatesBeatenToStringArray(checkmates_beaten) {
 ...
 */
function postCheckmateBeaten(req, res) {
    ...
    const checkmates_beaten_array = checkmatesBeatenToStringArray(checkmates_beaten);
    ...
    const updateSuccess = updateMemberColumns(user_id, { checkmates_beaten });
...
export { setPracticeProgressCookie, deletePracticeProgressCookie, getCheckmatesBeaten, postCheckmateBeaten };
