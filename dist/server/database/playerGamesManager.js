 ...
 */
function addGameToPlayerGamesTable(options) {
    const query = `
 ...
 */
function getPlayerGamesData(user_id, game_id, columns) {
    ...
    try {
        ...
        const row = db.get(query, [user_id, game_id]);
...
function getPlayersInGame(game_id) {
	...
	`;
    try {
        const entries = db.all(query, [game_id]);
        return entries;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
function updatePlayerGamesColumns(user_id, game_id, columnsAndValues) {
...
// Exports --------------------------------------------------------------------------------------------
export { addGameToPlayerGamesTable, getPlayerGamesData, getPlayersInGame,
