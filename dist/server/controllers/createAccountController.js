...
async function createNewMember(req, res) {
    ...
    if (!doUsernameFormatChecks(username, req, res))
        return;
    if (!doEmailFormatChecks(email, req, res))
        return;
    if (!doPasswordFormatChecks(password, req, res))
        return;
 ...
 */
async function generateAccount({ username, email, password, autoVerify = false }) {
 ...
 */
async function isEmailDNSValid(email) {
    try {
...
;
export { createNewMember, checkEmailValidity, checkUsernameAvailable, generateAccount };
