...
const variantDictionary = {
    ...
    '4x4x4x4_Chess': {
        generator: {
            algorithm: () => {
                return fourdimensionalgenerator.gen4DPosition(4, 4, 5, {
 ...
 */
function repeatPromotionsAllowedForEachColor(promotions, players = [p.WHITE, p.BLACK]) {
    const promotionRule = {};
...
function getStartingPositionOfVariant({ Variant, UTCDate, UTCTime }) {
    if (!isVariantValid(Variant))
        throw new Error(`Cannot get starting position of invalid variant "${Variant}"!`);
    ...
    if (variantEntry.positionString) {
        ...
        throw new Error(`Variant entry "${Variant}" NEEDS either a "positionString" or a "generator" property, cannot get the starting position!`);
}
/**
...
function getStartSnapshotPosition({ positionString, startingPosition, specialRights, pawnDoublePush = false, castleWith }) {
    if (positionString) {
        ...
        throw new Error("Not enough information to calculate the positionString, position, and specialRights of variant.");
...
function getVariantGameRuleModifications({ Variant, UTCDate = timeutil.getCurrentUTCDate(), UTCTime = timeutil.getCurrentUTCTime() }) {
    const variantEntry = variantDictionary[Variant];
    if (!variantEntry)
        throw Error(`Cannot get gameruleModifications of invalid variant "${Variant}".`);
...
function getGameRules(modifications = {}) {
    ...
    if (modifications.promotionRanks !== null) { // Either undefined (use default), or custom
        ...
        if (!modifications.promotionsAllowed)
            throw new Error("When overriding promotionRanks, you must also override promotionsAllowed!");
        gameRules.promotionsAllowed = modifications.promotionsAllowed;
    }
    if (modifications.moveRule !== null)
        gameRules.moveRule = modifications.moveRule || 100;
    // GameRules that DON'T have a dedicated ICN spot...
    if (modifications.slideLimit !== undefined)
        gameRules.slideLimit = modifications.slideLimit;
 ...
 */
function getMovesetsOfVariant({ Variant, UTCDate = timeutil.getCurrentUTCDate(), UTCTime = timeutil.getCurrentUTCTime() }, slideLimit) {
    ...
    if (!isVariantValid(Variant))
        throw new Error(`Cannot get movesets of invalid variant "${Variant}"!`);
    const variantEntry = variantDictionary[Variant];
    if (!variantEntry.movesetGenerator) {
        ...
        if (variantEntry.gameruleModifications?.hasOwnProperty(0)) { // Multiple UTC timestamps
            ...
            return getMovesets({}, slideLimit ?? variantEntry.gameruleModifications?.slideLimit);
        }
    }
...
function getSpecialMovesOfVariant({ Variant, UTCDate = timeutil.getCurrentUTCDate(), UTCTime = timeutil.getCurrentUTCTime() }) {
    ...
    if (!isVariantValid(Variant))
        throw new Error(`Cannot get specialMoves of invalid variant "${Variant}"!`);
    const variantEntry = variantDictionary[Variant];
    if (variantEntry.specialMoves === undefined)
        return defaultSpecialMoves;
...
function getSpecialVicinityOfVariant({ Variant, UTCDate = timeutil.getCurrentUTCDate(), UTCTime = timeutil.getCurrentUTCTime() }) {
    const defaultSpecialVicinityByPiece = specialmove.getDefaultSpecialVicinitiesByPiece();
    // Pasted games with no variant specified use the default
    if (Variant === undefined)
        return defaultSpecialVicinityByPiece;
    if (!isVariantValid(Variant))
        throw new Error(`Cannot get specialVicinity of invalid variant "${Variant}"!`);
    const variantEntry = variantDictionary[Variant];
    if (variantEntry.specialVicinity === undefined)
        return defaultSpecialVicinityByPiece;
